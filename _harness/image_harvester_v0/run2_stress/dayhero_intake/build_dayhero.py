...
BYTES = 2386153
LANE = 'pp-bg-dayhero-intake (2026-07-05)'
PATH = 'Property_Pulse/Backgrounds/bg_pp_perth_cbd_skyline_day_wide.jpg'
URL = 'https://mbkmaxqhsohbtwsqolns.supabase.co/storage/v1/object/public/brand-assets/' + PATH

# stage upload copy + verify bytes
src = '../final/perth_cbd_day_hero.jpg'
os.makedirs('upload_staging', exist_ok=True)
dst = 'upload_staging/bg_pp_perth_cbd_skyline_day_wide.jpg'
shutil.copy2(src, dst)
h = hashlib.sha256(open(dst, 'rb').read()).hexdigest()
assert h == SHA and os.path.getsize(dst) == BYTES, 'staged file hash/size mismatch — ABORT'

meta = {
 "mime": "image/jpeg", "bytes": BYTES, "width": 4000, "height": 2250, "sha256": SHA,
 "sha256_source": "cc-0027 run2_stress harvest file hashed at packet build 2026-07-05; upload target " + PATH + " must byte-match (in-SQL size precheck + mandatory post-upload public-URL sha256 check)",
 "usage": "background", "bucket": "brand-assets", "source_path": PATH, "asset_key": KEY,
 "location": "Perth", "aspect_ratio": "16:9", "has_people": False, "has_text": False,
 "visual_style": "photographic",
 "license": "Unsplash License (free for commercial use, no attribution required)",
 "license_type": "unsplash_license", "license_url": "https://unsplash.com/license",
 "attribution_required": False,
 "attribution_note": "Not required (optional credit: Photo by Joshua Leong on Unsplash)",
 "photographer": "Joshua Leong (@jleonnn)", "source_site": "unsplash",
 "source_url": "https://unsplash.com/photos/gl7nkS_h4lo",
 "original_download_url": "https://unsplash.com/photos/gl7nkS_h4lo/download?force=true",
 "approved": False, "approval_status": "intake_candidate", "production_use_allowed": False,
 "pk_decision": "intake_only (day-hero intake lane, PK best-pick acceptance 2026-07-05); production approval/promotion requires a separate PK gate — WARNING: since Option D (v4.95) the resolver pool drives LIVE PP image_quote production, so promotion of this asset changes production rotation",
 "visual_review_verdict": "PASS_WITH_NOTE",
 "visual_review_note": "cc-0027 run-2 stress proof: harvester best-pick, image-reviewer concurred PASS_WITH_NOTE (skyline construction cranes + small rooftop corporate logos [RioTinto + one LED sign] legible at full res = accepted trade-off, no agency branding, no faces); PK ACCEPTED as day-hero best-pick 2026-07-05 — closes the bright-day Perth skyline sourcing carry at the sourcing level.",
 "suggested_scrim_opacity": 0.48,
 "safe_for_text_overlay": "needs_scrim",
 "harvest_lane": "cc-0027 image-harvester run2_stress (2026-07-05)",
 "intake_lane_dayhero": LANE,
 "review_packet": "_harness/image_harvester_v0/run2_stress/dayhero_intake/DAYHERO_GATE_PACKET.md",
}

def jstr(o):
    return json.dumps(o, ensure_ascii=False, separators=(',', ':')).replace("'", "''")

apply_sql = f"""-- Day-hero intake: INSERT exactly 1 inactive intake-candidate row for {KEY}
...
BEGIN
  PERFORM 1 FROM storage.objects WHERE bucket_id='brand-assets'
    AND name='{PATH}' AND (metadata->>'size')::bigint = {BYTES};
...
SELECT '{AID}', '{PP}', 'other', 'Perth CBD bright-day skyline hero background',
       '{URL}',
       '{jstr(meta)}'::jsonb,
...
WHERE NOT EXISTS (
  ...
  WHERE client_id = '{PP}' AND asset_meta->>'asset_key' = '{KEY}'
);
...
BEGIN
  ...
  WHERE asset_id = '{AID}'
    ...
    AND asset_meta->>'intake_lane_dayhero' = '{LANE}'
...
COMMIT;
"""
io.open('dayhero_apply.sql', 'w', encoding='utf-8', newline='\n').write(apply_sql)

rollback_sql = f"""-- Day-hero intake ROLLBACK: delete the single candidate row IF still unpromoted (promotion-guarded).
...
BEGIN
  SELECT count(*) INTO remaining FROM c.client_brand_asset WHERE asset_id = '{AID}';
  IF remaining <> 0 THEN
...
COMMIT;
"""
io.open('dayhero_rollback.sql', 'w', encoding='utf-8', newline='\n').write(rollback_sql)

json.dump([{'upload_file': dst, 'target_path': 'brand-assets/' + PATH, 'bytes': BYTES, 'sha256': SHA}],
          io.open('upload_manifest.json', 'w', encoding='utf-8'), indent=1)

for f in ('dayhero_apply.sql', 'dayhero_rollback.sql'):
    print(f, hashlib.sha256(open(f, 'rb').read()).hexdigest(), os.path.getsize(f), 'bytes')
print('staged upload verified byte-exact:', dst)
