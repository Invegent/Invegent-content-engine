...
v4 (2026-08-07) — PK AURAL REVIEW COMPLETE. 13 -> 12 survivors (culled neutral_lofi_shimmer_021);
measured loudness_lufs now written into every row (was NULL); content_id_safe / all four fences
UNCHANGED (still false, still non-selectable). Every count in the harness derives from the manifest,
so the fenced-count assert, the rollback cardinality floor and the key lists all follow to 12
automatically. PK ruled NO facet corrections, so mood/energy/tempo_band/genre are untouched.
...
def build_apply(tracks):
    ...
    A("-- GENERATED from manifest.json by build_intake.py (v4) — do not hand-edit; regenerate instead.")
    A("-- v4: PK aural review complete (12 survivors, measured loudness_lufs written). Harness hardened")
    A("--     across three apply-harness-auditor shadow runs — see the generator docstring.")
    A("--")
    ...
    A(f"--   same {n} track_keys this file inserts). Storage objects are LEFT IN PLACE for re-attempt")
    ...
    for t in tracks:
        ...
        A("END $$;")
        A("WITH ins AS (")
        A("  INSERT INTO m.music_track (track_key, title, source, storage_bucket, storage_path, sha256, mime, bytes, duration_seconds, loudness_lufs, mood, energy, tempo_band, genre, vocals, approval_status, approved, production_use_allowed, is_active, notes)")
        A("  SELECT " + ", ".join(q(x) for x in [
            tk, mt["title"], mt["source"], mt["storage_bucket"], mt["storage_path"],
            mt["sha256"], mt["mime"], mt["bytes"], mt["duration_seconds"],
            # MEASURED (EBU R128 integrated) — was NULL in v3. Selection-neutral: the live winner
            # calm_piano_drifting_006 is -27.2 LUFS and still sorts first under ORDER BY ... ASC.
            mt["loudness_lufs"],
            mt["mood"],
    ...
    A(f"  --    ({n} minus any already present), so nothing else was created or destroyed.")
...
def build_rollback(tracks):
    ...
    A("-- GENERATED by build_intake.py (v4) alongside music_v1_batch2_intake_apply.sql.")
    A("--")
    A(f"-- IDENTITY: keyed on the SAME {n} track_keys the apply inserts by (AHA-07-2). It is NOT keyed")
    ...
    A("--")
    A(f"-- ⚠ STORAGE DISPOSITION (PK ruling 2026-08-06): the {n} uploaded objects in the PUBLIC")
