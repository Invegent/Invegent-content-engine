...
**Status:** OPEN — Phase 1 (5 GREEN), Phase 1b (`store_linkedin_org_token` search_path), the `upsert_publish_profile` revoke, Phase 2 Batch-1 (6 pipeline/notifier/crosspost RPCs), Phase 2 Batch-2 (9 internal-writer RPCs), and Phase 2 Batch-3 (8 dashboard/compliance/avatar RPCs) all APPLIED + VERIFIED (2026-06-16), plus Phase 2 Batch-4 (8 compliance/insights/dashboard/HeyGen-writer RPCs) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5A (8 Feeds+Drafts grant-only SECDEF signatures, highest-confidence dual-evidenced subset) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5C (`draft_approve_and_enqueue(uuid)`, AMBER→GREEN re-promoted) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5D (2 of 3 Lane A series-action functions: `approve_series_outline`, `set_episode_schedule` — grant cleanup after the Lane A server-side migration, dashboard deploy `47121c4`) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5D held-function (`draft_approve_and_enqueue_scheduled`, reachability/dead-path basis) APPLIED + VERIFIED (2026-06-17) — completing the Lane A series-action set (3/3), and Lane B grant-revoke (`update_onboarding_submission`, `validate_update_token` — portal onboarding-update, post-Lane-B-migration cleanup, portal deploy `44d4bb4`, smoke PASS) APPLIED + VERIFIED (2026-06-17) — closing the RED code-migration lane (all browser/anon SECDEF callers now server-side + grant-revoked); remaining systemic SECURITY DEFINER surface still open.
...
**Next gated step:** (1) **optional 0011 `search_path` riders** — `crosspost_facebook_to_linkedin`, `upsert_publish_profile`, `insert_ai_diagnostic_report`, `auto_resolve_pipeline_incidents`, `mark_compliance_review`, the 4 Batch-4 HeyGen writers (`complete_avatar_training`, `fail_avatar_generation`, `save_avatar_generation`, `store_gen_poll_response`), the 5 Batch-5A NULL-search_path signatures (`feed_assign_client`, `feed_unassign_from_client`, `feed_deactivate`, `review_feed_suggestion`, `draft_set_status`), `draft_approve_and_enqueue` (Batch-5C, NULL search_path), the 3 Batch-5D series-action functions (`approve_series_outline`, `set_episode_schedule`, `draft_approve_and_enqueue_scheduled`), and the 2 Lane B onboarding-update functions (`update_onboarding_submission`, `validate_update_token`) — all NULL search_path (low severity; bodies already safe); (2) **remaining-surface remediation** — Phase 2 Batch-5B (7 zero-runtime Feeds+Drafts signatures, repo-evidenced, deferred pending telemetry/PK) and Phase 2 Batch-6 (remaining GREEN: onboarding/series/subscriptions admin); the **RED code-migration lane is COMPLETE** — Lane A (deploy `47121c4`, 3 dashboard series fns) + Lane B (deploy `44d4bb4`, 2 portal onboarding-update fns) both migrated server-side and grant-revoked. Separately, a **series-v2 episode↔draft back-fill** engineering brief remains open (product bug, out of SEC scope); (3) **standard governance path** thereafter.
...
**Phase 2 Batch-5D held-function — `draft_approve_and_enqueue_scheduled(uuid,timestamptz)` — APPLIED + VERIFIED (2026-06-17), reachability/dead-path basis:** migration `sec_d_2026_06_16_002_phase2_batch5d_held_revoke_draft_approve_and_enqueue_scheduled` (version `20260617051627`) via Supabase `apply_migration`; review `7c0235f9-a997-4e26-89ea-f04f9dba1993` (`agree` / medium / high / **proceed** — no escalation); PK approval phrase received. **REVOKE EXECUTE FROM PUBLIC, anon, authenticated** (service_role retained; no body / search_path / schema / data change). **Basis (smoke architecturally unnecessary):** Lane A routes any future Series-UI caller as service_role; the sole caller (Series Approve button) is effectively dead/transient per the **Gate-0** read-only check (series-v2 slot-fill drafts are auto-approved by `auto-agent-v1` ~10 min after creation — 114 of 124 — none lingering in `needs_review`; the auto-approver does **not** use this function, its service_role count = 0); `authenticated`=22 historical/flat since the Lane A deploy (old `SeriesDetail.tsx` browser path removed by `216fe07`); `anon`=0. **Verification (read-only):** now `anon=false / authenticated=false / service_role=true` (ACL `{postgres, service_role}`); security advisor anon/auth SECDEF-executable **cleared**; residual advisor 0011 (NULL search_path) — deferred 0011 lane. **Rollback:** `GRANT EXECUTE ON FUNCTION public.draft_approve_and_enqueue_scheduled(uuid, timestamptz) TO anon, authenticated;` (no data impact). **Supersedes the earlier HELD note** (the prior 3-fn review `72257c81` escalated specifically on this function; the 2-fn split shipped the other two; this completes the set on the strengthened reachability basis). The series-v2 episode↔draft back-fill remains a separate engineering brief (product bug, out of SEC scope; this revoke is correct before and after that fix). **Lane A series-action set now COMPLETE (3/3). Branch B remains NOT AUTHORISED.**
**Lane B grant-revoke — onboarding-update functions (`update_onboarding_submission(uuid,uuid,jsonb)`, `validate_update_token(uuid,uuid)`) — APPLIED + VERIFIED (2026-06-17):** migration `sec_d_2026_06_16_002_lane_b_grant_only_revoke_onboarding_update_fns` (version `20260617073404`) via Supabase `apply_migration`; review `afef34b4-d4c4-430a-bdcc-9dc1f8d0e079` (`agree` / medium / high / **proceed** — no escalation); PK approval phrase received. *(First apply attempt hit a transient `fetch failed`; CCD verified read-only that nothing had applied + the migration name was unregistered, then retried safely — single registration confirms no double-apply.)* **REVOKE EXECUTE FROM PUBLIC, anon, authenticated** (service_role retained; no body / search_path / schema / data change). **Lane B code (portal deploy `44d4bb4`):** the two browser/anon callers in `app/onboard/update/page.tsx` were moved behind `service_role` server actions in `actions/onboarding.ts` (page review `9615ec40` → PK-gated merge → Vercel `portal.invegent.com`). **Production smoke PASS** (throwaway token-gated test submission `69bb19b4…`): both RPCs executed as `service_role` (Supabase API-log UA=`node`, zero browser-UA calls); `pg_stat_statements` service_role +1 each, `anon` flat (validate 3 historical / submit 0); single-use token preserved (token nulled + `missing_fields` cleared post-submit; reopen showed expired/invalid). **Verification (read-only):** both now `anon=false / authenticated=false / service_role=true` (ACL `{postgres, service_role}`); security advisor anon/auth SECDEF-executable **cleared** for both; residual advisor 0011 (NULL search_path) on both — deferred 0011 lane. **Rollback:** per-function `GRANT EXECUTE ON FUNCTION public.<fn>(<sig>) TO anon, authenticated;` (no data impact). Token single-use logic is in-body and untouched. **This was the last RED function — the RED code-migration lane is now COMPLETE (Lane A 3/3 + Lane B 2/2). Branch B remains NOT AUTHORISED.**
