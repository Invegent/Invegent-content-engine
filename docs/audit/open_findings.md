...
**Status:** OPEN — Phase 1 (5 GREEN), Phase 1b (`store_linkedin_org_token` search_path), the `upsert_publish_profile` revoke, Phase 2 Batch-1 (6 pipeline/notifier/crosspost RPCs), Phase 2 Batch-2 (9 internal-writer RPCs), and Phase 2 Batch-3 (8 dashboard/compliance/avatar RPCs) all APPLIED + VERIFIED (2026-06-16), plus Phase 2 Batch-4 (8 compliance/insights/dashboard/HeyGen-writer RPCs) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5A (8 Feeds+Drafts grant-only SECDEF signatures, highest-confidence dual-evidenced subset) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5C (`draft_approve_and_enqueue(uuid)`, AMBER→GREEN re-promoted) APPLIED + VERIFIED (2026-06-17), Phase 2 Batch-5D (2 of 3 Lane A series-action functions: `approve_series_outline`, `set_episode_schedule` — grant cleanup after the Lane A server-side migration, dashboard deploy `47121c4`) APPLIED + VERIFIED (2026-06-17), and Phase 2 Batch-5D held-function (`draft_approve_and_enqueue_scheduled`, reachability/dead-path basis) APPLIED + VERIFIED (2026-06-17) — completing the Lane A series-action set (3/3); remaining systemic SECURITY DEFINER surface still open.
