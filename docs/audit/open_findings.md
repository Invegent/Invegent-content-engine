...
**Status:** OPEN — Phase 1 lockdown APPLIED + VERIFIED (5 GREEN functions) and Phase 1b `search_path` hardening APPLIED + VERIFIED (`store_linkedin_org_token`), both 2026-06-16; remaining systemic surface still open.
...
**Next gated step:** (1) **`upsert_publish_profile` standalone PK-gated revoke** — MANUAL-ONLY (resolved 2026-06-16; see classification note below): `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` (retain postgres + service_role) via its own D-01 + PK phrase, **NOT bundled with Phase 2**; (2) **Phase 2** — batched lockdown of the remaining systemic SECURITY DEFINER functions (each caller-confirmed + D-01); (3) **standard governance path** thereafter.
...
**`upsert_publish_profile` — classification: MANUAL-ONLY (2026-06-16; AMBER resolved).** `public.upsert_publish_profile(uuid,text,text,text,text,text,bool,bool)` — single overload, **SECURITY DEFINER**, owner **postgres**; writes **`c.client_publish_profile`** (incl. token-like `page_access_token`) via `ON CONFLICT (client_id, platform)`. **Grants still include PUBLIC/anon/authenticated/service_role** (Phase 1 did not touch it); **advisors 0028/0029/0011 still active.** **Caller audit (db-rls-auditor `aee385136f4959dcc` + security-auditor `ad006b3dc97129511`):** **zero runtime RPC callers** across content-engine / dashboard / portal / web; the only evidenced invocation is the **manual `SELECT public.upsert_publish_profile(...)`** onboarding SQL in `docs/briefs/brief_042_linkedin_zapier_bridge.md` (run as postgres in the SQL editor); **Zapier is outbound-only** (the `linkedin-zapier-publisher` EF reads the webhook URL and POSTs out — it does not call this RPC). **Recommendation (PK-gated, NOT applied):** a **standalone** `REVOKE EXECUTE … FROM PUBLIC, anon, authenticated` (retain postgres + service_role), via its own D-01 + PK phrase — **not bundled with Phase 2.** Retaining postgres preserves the brief_042 manual onboarding path. Optional rider: pin `search_path` (advisor 0011; low severity — body ref already schema-qualified).
