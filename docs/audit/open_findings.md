...
**Status:** OPEN — Phase 1 (5 GREEN), Phase 1b (`store_linkedin_org_token` search_path), the `upsert_publish_profile` revoke, Phase 2 Batch-1 (6 pipeline/notifier/crosspost RPCs), Phase 2 Batch-2 (9 internal-writer RPCs), and Phase 2 Batch-3 (8 dashboard/compliance/avatar RPCs) all APPLIED + VERIFIED (2026-06-16), plus Phase 2 Batch-4 (8 compliance/insights/dashboard/HeyGen-writer RPCs) APPLIED + VERIFIED (2026-06-17), and Phase 2 Batch-5A (8 Feeds+Drafts grant-only SECDEF signatures, highest-confidence dual-evidenced subset) APPLIED + VERIFIED (2026-06-17); remaining systemic SECURITY DEFINER surface still open.
...
**Next gated step:** (1) **optional 0011 `search_path` riders** — `crosspost_facebook_to_linkedin`, `upsert_publish_profile`, `insert_ai_diagnostic_report`, `auto_resolve_pipeline_incidents`, `mark_compliance_review`, the 4 Batch-4 HeyGen writers (`complete_avatar_training`, `fail_avatar_generation`, `save_avatar_generation`, `store_gen_poll_response`), and the 5 Batch-5A NULL-search_path signatures (`feed_assign_client`, `feed_unassign_from_client`, `feed_deactivate`, `review_feed_suggestion`, `draft_set_status`) (low severity; bodies already safe); (2) **remaining-surface remediation** — Phase 2 Batch-5B (7 zero-runtime Feeds+Drafts signatures, repo-evidenced, deferred pending telemetry/PK), the `draft_approve_and_enqueue(uuid)` **AMBER** caller-investigation (8 `authenticated` `pg_stat_statements` executions — hidden client-side caller), Phase 2 Batch-6 (remaining GREEN: onboarding/series/subscriptions admin), and the **RED code-migration lane** (`update_onboarding_submission`, `approve_series_outline`, `set_episode_schedule`, `draft_approve_and_enqueue_scheduled` — move browser callers server-side first); (3) **standard governance path** thereafter.
