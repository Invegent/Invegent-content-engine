...
**Next gated step:** (1) **`upsert_publish_profile` AMBER resolution** — confirm manual-SQL-only (no anon/authenticated REST caller) before any lockdown; (2) **Phase 2** — batched lockdown of the remaining systemic SECURITY DEFINER functions (each caller-confirmed + D-01); (3) **standard governance path** thereafter.
