...
**Next gated step:** (1) **optional** — pin `search_path` on `upsert_publish_profile` (advisor 0011 rider; low severity — body ref already schema-qualified); (2) **Phase 2** — batched lockdown of the remaining systemic SECURITY DEFINER functions (each caller-confirmed + D-01); (3) **standard governance path** thereafter.
