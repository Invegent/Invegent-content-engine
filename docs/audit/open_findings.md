...
**Next gated step:** (1) **optional 0011 `search_path` riders** — `crosspost_facebook_to_linkedin`, `upsert_publish_profile`, `insert_ai_diagnostic_report`, `auto_resolve_pipeline_incidents`, `mark_compliance_review`, and the 4 Batch-4 HeyGen writers (`complete_avatar_training`, `fail_avatar_generation`, `save_avatar_generation`, `store_gen_poll_response`) (low severity; bodies already safe); (2) **remaining-surface census → Phase 2 Batch-5 discovery** (classify + caller-confirm the next tranche of the systemic SECDEF surface); (3) **standard governance path** thereafter.
