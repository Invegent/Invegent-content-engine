...
> **✅ v3.06 deployed↔repo drift close (2026-05-25):** Source-of-truth reconciliation — **repo-only, 0 production mutation, 0 D-01.** (1) **instagram-publisher synced to repo**: deployed v2.4.0 was **4 versions ahead** of repo v2.0.0 — pushed the exact deployed source (`bc78511e`, byte-verified on every anchor; rollback hazard closed — a redeploy from repo no longer reverts the fix). (2) **7 chat-applied 2026-05-24 migrations backfilled** to `supabase/migrations/` (`8738c3a0`, recovered verbatim from `supabase_migrations.schema_migrations.statements`) — the IG fresh-restart trio + cc-0019 gate + the 3 YT v3.05 migrations; **DB→repo migration drift CLOSED**. (3) **F-YT-FAILED-NO-RETRY brief authored** (`docs/briefs/yt-publisher-failed-no-retry.md`, `bee872aa`), modelled on the IG v2.4.0 bounded-retry + channel-hold pattern. (4) **cc-0019 status CORRECTED**: live DB has `m.is_publish_eligible(uuid,text)` + the `m.fill_pending_slots` gate (migration `20260524091020`; governed D-01 `a2e5f3f5` + PK approval per the migration header) → **Unit A IMPLEMENTED**; but the deployed **ai-worker v2.12.0** has no eligibility preflight → **Unit B NOT implemented**. v3.05's "cc-0019 brief only, NOT implemented" was **stale** (corrected here; not reconstructing the exact apply session). The pre-compaction **IG-publish-restore arc OWED 4-way close is now substantially discharged** (publisher-code + migration-file legs); residual = cron 53/64 operational state (already documented) + PK's IG-resume decision. Session `1219aff0`. Repo now: `instagram-publisher` = deployed v2.4.0; all 7 migration files present. Next: fire the YT F-YT-FAILED-NO-RETRY `plan_review` D-01. Dashboard PHASES leg N/A (operational).
...
### 2026-05-25 Sydney — v3.06: deployed↔repo drift close (IG publisher + 7 migration backfills) + F-YT-FAILED-NO-RETRY brief + cc-0019 correction

**Outcome:** Source-of-truth reconciliation, **repo-only — 0 production mutation, 0 D-01.** A deep audit of the 24 May YT + IG fixes surfaced that deployed `instagram-publisher` was **v2.4.0** while repo `main` was **v2.0.0** (a 4-version drift where a redeploy would silently revert the fix). Actions: (1) pushed the exact deployed v2.4.0 source to repo (`bc78511e`, byte-verified on every anchor — rollback hazard closed); (2) backfilled 7 chat-applied 2026-05-24 migration files to `supabase/migrations/` (`8738c3a0`, recovered verbatim from `schema_migrations.statements`) — DB→repo migration drift closed; (3) authored the F-YT-FAILED-NO-RETRY brief (`bee872aa`); (4) **corrected cc-0019**: verified from the live DB that `m.is_publish_eligible` + the `fill_pending_slots` gate exist (Unit A IMPLEMENTED, migration `20260524091020`, governed D-01 `a2e5f3f5` + PK approval per header), and from the deployed **ai-worker v2.12.0 source** that there is no eligibility preflight (Unit B NOT implemented). v3.05's "brief only" was stale.
...
- **F-YT-FAILED-NO-RETRY brief — AUTHORED v3.06** (`docs/briefs/yt-publisher-failed-no-retry.md`, `bee872aa`); `plan_review` D-01 next, then ef_deploy + sql_destructive (Unit B = bounded recovery of the 30 failed drafts) + PK approval.
...
Carries: cc-0015 (Wave 7, gated on Gate 11 2026-05-26); cc-0016 Stage E (future/separately-approved); **Q-005 (OPEN, next-fire watch; brief v3.1.1)**; **YT backlog drain watch / YT token durability / CFW+Invegent YT onboarding / F-YT-FAILED-NO-RETRY brief (AUTHORED v3.06) / F-YT-EXPIRY-DISPLAY-FAKE cleanup / cc-0019 Unit B optional (Unit A implemented)**; cc-0017c v1.2 doc patch; cc-0017a v1.2 doc patch; vchecks.md V-B4 doc patch; minor doc patches; F-CRON-AUTO-APPROVER-SECRET-INLINE; backend/shared-metrics refactor (deferred); lesson promotions (L-v2.83-a STRONG 18+; **L-v2.85-e PROMOTION-CONFIRMED**; L-v2.85-a HIGH-SIGNAL 4 occurrences; L62 strongly reinforced; L-v2.88-a watcher; **L-v2.94 convention NEW candidate**; **L-v3.02-a / L-v3.03-a / L-v3.04-a NEW candidates**; **L-v3.05-a NEW candidate — record the root CAUSE not just the symptom**; **L-v3.06-a NEW candidate — verify deployed-vs-repo state before redeploy + backfill chat-applied migrations same-session**; **L41 re-exercised v3.02.1 + reinforced v3.03/v3.04/v3.05/v3.06**; L-v2.90-a-f watchers).
...
- **F-YT-FAILED-NO-RETRY brief authored** (`bee872aa`); `plan_review` D-01 is the next step.
...
- **youtube-publisher v1.6.0** unchanged. **F-YT-FAILED-NO-RETRY**: its catch handler sets transient auth failure to terminal `failed` (never re-selected). Reliability gap — brief authored v3.06 (`bee872aa`); do NOT treat `failed` YT drafts as auto-recoverable.
...
- **T-MCP-02 quota: ~96 cumulative (v3.05 +4 D-01: 2 plan_review cc-0019 + 2 sql_destructive YT)**, ON TOP of an estimated ~+5 from the unrecorded pre-compaction IG arc → **treat cumulative as needing a clean re-baseline (~101–105 range)**, not a precise figure. **v3.06 added 0 D-01.** State-capture exceptions: 1 unchanged.
