...
- **cc-0019** — **Unit A IMPLEMENTED** (live `is_publish_eligible` + `fill_pending_slots` gate; migration `20260524091020`; D-01 `a2e5f3f5` + PK approval per header; repo-backfilled `8738c3a0`). **Unit B (ai-worker preflight) NOT implemented** (absent in deployed ai-worker v2.12.0) — optional; the fill-gate already stops the main waste vector. Implement Unit B only if PK wants the defence-in-depth (ef_deploy D-01 + approval). Do NOT re-apply Unit A.
...
- **cc-0019 — Unit A IMPLEMENTED v3.06 (correction).** Live DB has `m.is_publish_eligible` + the `m.fill_pending_slots` gate (migration `20260524091020`; D-01 `a2e5f3f5` + PK approval per header; repo-backfilled `8738c3a0`). **Unit B (ai-worker preflight) NOT implemented** (absent in deployed ai-worker v2.12.0). v3.05's "not created" was stale. **Do NOT re-apply Unit A (already live).** Unit B optional.
