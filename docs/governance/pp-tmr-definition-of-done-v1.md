...
**✅ D4 CLOSED — 2026-07-17 (v5.65).** Apply B deployed `tmr-drift-probe` v2.1.0 (check (d), commit `651362b`) and its first run confirmed `declarative_coverage.status='ok'` (must_declare 17 == declared 17, 0 violations, `registry_source='github:main'` v0.10) — the D4 invariant is continuously machine-checked and holds. **D4 is CLOSED.** (Ultimate still gated on D6, and the other D1/D2/D3/D7 targets.) Record: `docs/briefs/results/d4-close-lane-result.md`.
