# Post-CGU-v1 — Phase 2 Schedule Expansion — Apply Packet v2
...
**Supersedes v1** (hash `5e0cb969797b3816a8e478c0cfe9f986227e22eec9a54172459297ec40bb05eb`,
now execution-ineligible). v1 was never edited in place — v2 is a new frozen file. v1
was reviewed by `apply-harness-auditor` and returned **CONCERNS** with 5 findings
(AHA-07-1 high, AHA-08-1 medium, AHA-06-1 medium, AHA-01-1 low, AHA-07-2 low); v2 fixes
all five:

1. **AHA-07-1 (high, fixed):** `_pre_image` and `_expected_ownership` were two
   independently-hand-typed 17-row literal lists with no runtime check that they're the
   same set — a silent divergence would have left the durable rollback table holding
   pre-image data for a different row-set than what was actually mutated. §3 step 5 now
   adds an explicit symmetric-difference assertion (must be 0 rows).
2. **AHA-08-1 (medium, fixed):** the old §5 listed the rollback-table-collision STOP
   last, but it fires second in actual execution order (right after client-identity
   resolution, before the CAS guard). §5 below is reordered to match true execution
   order and each item now names its enforcing SQL location.
3. **AHA-01-1 (low, fixed):** the old §5 header claimed to be the complete list of
   everything machine-enforced, but omitted Protection A's 12 client-resolution abort
   paths and 5 pre-check baselines. §5's header is reworded to be accurate, and those
   paths are now explicitly named.
4. **AHA-06-1 (medium, fixed):** CFW's LinkedIn baseline covered only the `image_quote`
   subset, and invegent's LinkedIn plus both clients' YouTube had no independent
   baseline at all — the "zero touch" claim rested on structural UPDATE scoping alone,
   with no defense-in-depth counter-check (unlike PP and NDIS, which already had one).
   §3 step 4 now adds `_cfw_li_total_baseline`, `_invegent_li_baseline`,
   `_cfw_yt_baseline`, `_invegent_yt_baseline`, each captured pre-UPDATE and
   re-verified post-UPDATE.
5. **AHA-07-2 (low, fixed):** the §4 rollback script had no row-count assertion. It now
   uses `GET DIAGNOSTICS` and aborts if the restore doesn't affect exactly 17 rows.

Everything else — the 17-row manifest, target formats, before/after volume matrix, the
guarded UPDATE itself, the no-reopened-v11-row guard, the zero-tolerance readiness
check — is **byte-identical in effect** to v1; only the additional guard rails above
were added.

---

## 1. Exact row-level manifest (17 rows) — unchanged from v1
...
## 2. Before / after volume matrix — unchanged from v1
...
-- ══ 4. Pre-existing invariant baselines (must not move — defense in depth). Fixes
-- AHA-06-1: every client+platform this packet claims "zero touch" for now gets an
-- independent baseline, not just the ones structurally scoped away from the UPDATE. ══
...
BEGIN
  ...
  -- Fixes AHA-07-1: _pre_image and _expected_ownership must be the exact same 17-row
  -- set (they were independently hand-typed at authoring time; this asserts it, rather
  -- than trusting it) — otherwise the durable rollback table (sourced from _pre_image)
  -- could silently diverge from the row-set the UPDATE (driven by _expected_ownership)
  -- actually mutates.
...
if ever needed — now with a fail-closed row-count assertion (fixes AHA-07-2):
...
## 5. STOP conditions — reordered to match actual execution order (fixes AHA-08-1);
header reworded to accurately scope what is and isn't itemized here (fixes AHA-01-1)

The list below covers every **distinct row-set/aggregate-level** guard. It does not
re-itemize Protection A's 12 individual client-resolution abort paths (4 clients ×
{zero-rows, multi-row, drift} — all fire first, before anything else, per §3) or restate
each of the 9 individual baseline pre-checks in §3 step 5 one-by-one (they are the "any
baseline" entries below, itemized once).
...
5. `_pre_image` and `_expected_ownership` schedule_id sets differ (new in v2 — fixes AHA-07-1).
...
9. Any pre-apply baseline mismatch (PP total=16, PP carousel config=1, CFW LI image_quote=0, CFW LI total=5, invegent LI total=5, CFW YT=0, invegent YT=0, NDIS YT=0, NDIS LI=14 — 9 checks, new in v2: CFW LI total, invegent LI, CFW YT, invegent YT).
10. Any of the 17 rows fails to land on its exact frozen final state post-UPDATE.
11. Missing/extra check on the 17 rows post-UPDATE.
...
**Next steps (not yet done):** fresh `apply-harness-auditor` re-review on this v2 hash
(to confirm all 5 v1 findings are closed and no new ones introduced), `branch-warden`,
then a hash-pinned `ask_chatgpt_review` (mandatory — this packet is DML). Execution
requires a separate, explicit PK production-authorization message naming this exact
file and its frozen hash, per this lane's standing gate.
