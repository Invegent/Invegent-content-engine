# Gate Packet — Video Breadth Phase 2B · PP Candidacy Write-Set (DESIGN, HELD) — v2 (S3)

**Created:** 2026-07-25 Sydney · **Author:** S3 · **Status:** DESIGN packet v2 — FROZEN, **no DB write**. Stops at the 2B T3 DB-write gate.
**Supersedes:** v1 `cba1e9a9…` (reviewed by the 2B chain). **v2 applies the `apply-harness-auditor` (shadow) findings:** AHA-05-1 (single-call atomicity), AHA-07-1 (rollback identity derived inline), AHA-03-1 (executable `count(*)=1` template STOP), AHA-06-1 (untouched-claim narrowed). `db-rls-auditor` returned **PASS / no must-fix** on v1's write-set content (schema/constraint/FK/unique/RLS all conform; winner-unchanged confirmed live) — v2 changes only the harness packaging, not any column value, except aligning `evidence_kind` to the `local_render_file` exemplar convention.
**Base (ground-truthed):** origin/main == `330da9b` (authoritative; local HEAD `595e65d` = origin + **2 unpushed commits from OTHER lanes** — not mine, not touched/pushed). Design-only; no git mutation.
...
BEGIN
  ...
  IF (sel->'selected'->>'provider_template_id') <> 'c11bb8ab-18bd-45ff-aedd-0a59cb3773ab'
     OR NOT (sel->'alternatives') @> '[{"provider_template_id":"03bc6a3c-985a-4488-b008-67632372783c"}]'::jsonb THEN
...
--   expect: status=ok; selected.provider_template_id = c11bb8ab; alternatives=[{provider_template_id:03bc6a3c,
...
- **External review (v1):** `partial` / medium / high-confidence, **auto-escalated to PK** (review_id `c37c9532`, pinned `cba1e9a9`). Triage: **policy_decision** (touching PP's production winner is a PK judgment call) + **missing_evidence** (winner-guard/Logo sufficiency — since **closed** by db-rls-auditor's live confirmation). No concrete defect. **v2 changed the packet → the v1 review is STALE (hash rule 4); re-run pinned to the v2 hash before apply.**

**Remaining gates (PK's to open):** external re-review on the v2 hash → `branch-warden` (⚠ local ahead 2 = other lanes' unpushed commits — must NOT be swept into any commit) → **rollback validated before apply** → explicit PK apply gate. Apply as **one `execute_sql` call** under PK authorisation. Post-apply: run §5 probes; if P1 fails the winner-unchanged expectation → immediate rollback (§6). The policy_decision (live PP candidate on an enabled path) is inherently PK's to accept — which PK already did in ruling PP over NDIS.
