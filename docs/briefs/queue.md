...
|---|---|---|---|---|---|
| `nightly-health-check-v1` (v2 patch) | 0 | ready | cowork | 2026-05-02 | **v2 brief patched after v1 first-run learnings + B investigation.** Schema fixes (Q7 post_draft_id/client_slug/status='published'; Q9 decision/attempted_at; Q3/Q4 window_hours=24 filter). NEW Q-stuck (drill-down) + Q-true-stuck (actionable). NEW Section 6a/6b (categorisation Cat A/B/C). NEW Section 10 priority-tier flags. NEW S17 minimum-n guard (skip alert when calls < 10). v1 first run identified 5 LinkedIn-approved drafts at PP with publish_attempts=0 — v2 catches this pattern automatically as Cat C true-stuck. **PK must delete `docs/audit/health/2026-05-02.md` before re-firing today** to bypass idempotency check. Brief at `docs/briefs/nightly-health-check-v1.md`. |
