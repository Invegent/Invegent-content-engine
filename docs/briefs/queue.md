...
|---|---|---|---|---|---|
| `operator-alerting-trio-column-purposes` | 1 | review_required | cc | 2026-04-30 | R05 from action list. CC drafted 2026-04-30T065007Z — 57/57 HIGH, **0 LOW** (matches slot-core + pipeline-health-pair clean outcomes). `compliance_review_queue` 19/19 with `ai_analysis` JSONB schema cited in `supabase/functions/compliance-reviewer/index.ts:200-244` (keys: summary, relevance, confidence, key_changes, affected_rules[], new_rules_suggested[], human_action_required, confidence_reason) + `public.store_compliance_ai_analysis` RPC body confirmed. `external_review_queue` 21/21 + `external_review_digest` 17/17 sourced from D156 migration `supabase/migrations/20260421_d156_external_reviewer_layer.sql` CREATE TABLE + table COMMENT (severity info\|warn\|critical; trigger_type weekly_cron\|on_demand; status running\|succeeded\|failed). Pause context kept at table level only — no "(paused)" suffixes on column purposes. Awaiting chat MCP apply. |
