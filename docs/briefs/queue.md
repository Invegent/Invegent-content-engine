...
|---|---|---|---|---|---|
| `nightly-health-check-v1` (v3.0) | 0 | ready | cowork | 2026-05-02 | **Status reset 2026-05-20 v2.94 from `review_required` → `ready` per new recurring-brief lifecycle convention** (`docs/runtime/automation_v1_spec.md` Status flow §). Clean execution with operational findings routes findings to friction.* and returns the brief to `ready` for the next scheduled fire; it does not hard-block on PK approval. Q-nightly-health-check-v1-004 **RESOLVED 2026-05-20 PK Option A** — see `docs/runtime/claude_answers.md` A-nightly-health-check-v1-004 and back-pointer in `docs/runtime/claude_questions.md` Closed (resolution refs). Brief Section 6a Cat A wording simplified to "`profile_enabled=false` (regardless of platform)"; prior 2026-05-17 emissions stand as correct (no friction.event cleanup, no finding_id changes). **Prior 2026-05-17T16:02:10Z run preserved**: Cowork executed all 14 SQL queries verbatim (0 schema-drift fallbacks) + emitted 5 P1 findings to `friction.event` via `friction.fn_emit_health_check_findings` (success_count=5, failure_count=0, run_id `nightly-health-check-v1/2026-05-17T160210Z`). V-C3 (cc-0014 Stage C live verification) UNBLOCKED — markdown ↔ friction.event ID-level reconciliation now possible. Section 10 P1: 5 true-stuck clusters (linkedin × property-pulse n=8 grown from 5 on 2026-05-04 during the 12-day owner-gate skip; youtube × property-pulse n=7 oldest ~18 days; youtube × ndis-yarns n=5; instagram × care-for-welfare-pty-ltd n=2; instagram × invegent n=2 — both instagram clusters likely root-caused by `instagram-publisher-every-15m` jobid 53 being `is_active=false`). 0 P2 findings. Output `docs/audit/health/2026-05-17.md`, state `docs/runtime/runs/nightly-health-check-v1-2026-05-17T160210Z.md`. Findings belong in friction triage, not a hard block on the recurring brief. 9-of-10 success thresholds hit (Q-004 logged is the 1 miss; PK review time TBD). Prior context preserved: v3.0 patched 2026-05-15 cc-0014 Stage C; v2.1 brief-lock reopened to deliver dual-write; 12-day owner-gate skip 2026-05-05→2026-05-16. **Brief is eligible for the next 16:00 UTC Cowork fire.** |
