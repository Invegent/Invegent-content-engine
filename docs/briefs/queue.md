...
| `publish-queue-and-publish-column-purposes` | 1 | ready | cc | 2026-05-03 | **Authored chat-side 2026-05-03T00:35Z while waiting for F-PUB-006 Stage 3 verification.** Tier 1, follows established pattern (slot-core → post-publish-observability → pipeline-health-pair → operator-alerting-trio). Targets two 0%-documented tables: `m.post_publish_queue` (20 cols) + `m.post_publish` (15 cols) = 35 cols total. F-PUB-006 work area — context fresh. Producer code map embedded in brief: `m.enqueue_publish_from_ai_job_v1()`, `m.publisher_lock_queue_v2()`, publisher EFs (facebook + linkedin-zapier + instagram). HIGH/LOW classification + 0-2 LOW budget per pattern. Brief at `docs/briefs/2026-05-03-publish-queue-and-publish-column-purposes.md`. Estimated 70-95 min CC closure. **Side note for next session**: pre-flight discovered `m.ai_job` shows only 3/20 documented despite slot-core claiming 20/20 — possible coverage regression worth investigating separately. |
