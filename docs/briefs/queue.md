...
|---|---|---|---|---|---|
| `nightly-health-check-v1` (v3.1.1) | 0 | review_required | cowork | 2026-05-02 | **v3.1.1 first scheduled fire executed 2026-05-21T16:03:02Z.** All 14 SQL queries ran verbatim — 0 schema-drift fallbacks, 0 SQL-syntax bugs. Section 10: 5 P1 true-stuck clusters + 1 P2 (`zero-counts-pub-published-30m`). **Emission CLEAN: success_count=5 failure_count=0 skipped_count=0** — the 5 P1 true-stuck findings emitted to `friction.event` with `condition_key=true_stuck` (v3.1.1 emission_rule guard held; known-good P1 path did NOT regress vs 2026-05-17/2026-05-20; 5 rows verified present in `friction.event`). 1 P2 (`zero-counts-pub-published-30m`) correctly PARKED — surfaced in markdown, omitted from emission per §12.2a (EXPECTED interim behaviour, not a defect; `omitted_unsupported_types=zero_count_metric_missing`). **Notable signal:** the two instagram true-stuck clusters surged sharply day-over-day — instagram×invegent 2→16 (+14), instagram×care-for-welfare-pty-ltd 2→13 (+11); root cause jobid 53 `instagram-publisher-every-15m` `is_active=false` (no consumer drains the instagram queue). Other 3 clusters near-flat (youtube×ndis-yarns 8→9, youtube×property-pulse 7→7, linkedin×property-pulse 5→5; oldest ~22d). 1 new question **Q-nightly-health-check-v1-006** — Q-stuck returned an 8th row (`facebook×care-for-welfare-pty-ltd` n=3) matching none of Cat A/B/C (approved + profile_enabled=true + zero_publish_attempts=0); default-and-continue applied (default Option B: report as Unclassified, non-actionable, no brief change). S17 escalation rate exactly 40.0% — at but not above the strict >40% flag, so no P2 (vs 40.91% breach on 2026-05-20). Markdown `docs/audit/health/2026-05-21.md` canonical. Run state `docs/runtime/runs/nightly-health-check-v1-2026-05-21T160302Z.md`. **Q-005 remains OPEN** — a clean v3.1.1 interim fire does NOT by itself close it (see brief close-out gate: needs either `emission_rule` seed patch + full §12.2a mapping restore, or formal narrowing to P1-true-stuck-only). **Next:** PK reviews 5 P1 friction.event rows at `/operations`, decides jobid 53 re-activation / instagram-queue drain, resolves Q-006, progresses Q-005; on resolution sets brief status back to `ready` for next scheduled fire. |
