...
|---|---|---|---|---|---|
| `nightly-health-check-v1` (v2.1) | 0 | ready | cowork | 2026-05-02 | **v2.1 scheduled run #1 complete 2026-05-04T105109Z** — 5-of-7 thresholds hit at run-end (0 questions, 0 corrections, 0 schema bugs, 0 production writes; remaining 2 thresholds depend on PK review time + Section 10 surfacing accuracy, accepted by PK 2026-05-04). Output `docs/audit/health/2026-05-04.md`, state `docs/runtime/runs/nightly-health-check-v1-2026-05-04T105109Z.md`. **3 Priority 1 clusters filed for tomorrow's comparison run**: linkedin×PP (2 items, ~2.5d residual from 2 May v2-patch trigger), youtube×PP (2, NEW from 5-3), youtube×NY (1, NEW today — YouTube is ingest-only architecturally, indicates upstream config bug). **Secondary**: S17 escalation rate 52% (13/25 calls in 7d) > 40% threshold (T-MCP-06 signal). **Queue row reset `review_required` → `ready` 2026-05-04 11:30 UTC** — manual recurring-brief convention (auto-reset = future refinement candidate). Next nightly fire ~02:00 AEST 5 May = 16:00 UTC 4 May. |
