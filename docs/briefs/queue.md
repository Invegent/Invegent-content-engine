...
|---|---|---|---|---|---|
| `nightly-health-check-v1` | 0 | review_required | cowork | 2026-05-02 | **First run executed 2026-05-02T064839Z by Cowork.** Output: `docs/audit/health/2026-05-02.md` (11 sections, 12 queries, 0 production writes). Hit 4 brief-author schema bugs (Q7: `draft_id`→`post_draft_id`, `c.slug`→`client_slug`, status enum `success`→`published`; Q9: `outcome`→`decision`, `created_at`→`attempted_at`) — all recovered via `information_schema` lookup; logged as `Q-nightly-health-check-v1-001`. Also handled `m.cron_health_snapshot` dual-window shape (Section 4 defaulted to `window_hours=24`). All 4 measurable success thresholds hit (questions=1 ≤ 5; defaults overridden=0; output produced; production writes=0); PK review pending. Notable signals: `has_stuck_items=true` persistent ≥3h; `m.ai_job` + `m.slot_fill_attempt` zero in 24h (expected per memory); 6/6 publishes routed to `property-pulse` (NDIS-Yarns/CFW/Invegent zero); ChatGPT Review escalation rate 50% over 7d (n=2). Run state: `docs/runtime/runs/nightly-health-check-v1-2026-05-02T064839Z.md`. |
