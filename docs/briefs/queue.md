...
|---|---|---|---|---|---|
| `nightly-health-check-v1` (v3.0) | 0 | ready | cowork | 2026-05-02 | **v3.0 reset 2026-05-17 — ready for next Cowork fire.** Brief patched v2.1 → v3.0 on 2026-05-15 Sydney (cc-0014 Stage C: adds dual-write to `friction.event` via `friction.fn_emit_health_check_findings`; brief at HEAD `bc32e86`). **Q-003 disposition: Cowork used operational/Sydney filename reading (`2026-05-05.md`) under default-and-continue; no further owner correction is required before the next v3.0 fire.** **LinkedIn-NY cluster from the 2026-05-04 run remains carried forward for the next health-check cycle.** Prior `review_required` from the 2026-05-04 v2.1 run carried unchanged through the v3.0 patch; this reset releases the owner-gate after 12 consecutive `no-ready-briefs` skips (2026-05-05 → 2026-05-16). Last successful output: `docs/audit/health/2026-05-05.md`, state `docs/runtime/runs/nightly-health-check-v1-2026-05-04T160846Z.md`. Next Cowork fire (~16:00 UTC = ~02:00 AEST next day) will execute v3.0 end-to-end including the emission step. **V-C3 (cc-0014 Stage C live verification)** depends on this run producing ≥1 P1+P2 finding so finding_ids in the markdown can be reconciled against `friction.event` rows for the captured `run_id`. |
