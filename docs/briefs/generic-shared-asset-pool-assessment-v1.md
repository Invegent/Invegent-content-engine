...
### 1.7 Governing rule for sensitive / consent-bound imagery — **NEW · ratified 2026-07-19 (register v5.73)**

As of 2026-07-19 the sensitive-imagery exclusion has a real governing citation: **NDIS content Rule 3.5** (`docs/compliance/ndis_content_rules.md` Rule Group 3). It holds that an identifiable image of a real NDIS participant — or of an identifiable person depicted as a person with disability — is personal information under Code of Conduct Rule 2 (Provider Guidance para 31-35 / para 32) and may not be used without documented, verifiable consent; such imagery is consent-bound to the client that holds the consent and is therefore **structurally ineligible for any shared or reused generic pool**. This **replaces the earlier unbacked seed reference** (*"v5.79 NDIS Sensitive-Real-Imagery policy"*, grep-confirmed never to have existed) that risk 4.3 (§4) previously flagged as OQ-1.

- **Scope (current-state fact):** Rule 3.5 governs the participant-privacy/consent dimension **only** — **analogy-grounded** (no imagery-specific NDIS paragraph on file; a knowing PK election). Brand- and affiliation-neutrality of pooled imagery (no NDIS logo / civic-organisational cue / competitor branding) stays with the `brand_neutral` mechanism — **risk 4.1**, not this rule.
- **Rule vs fence (do not conflate):** the *rule* is now **current/ratified**; the *enforcing fence* at the pool boundary (`brand_neutral=false` + policy fence keeping such assets client-scoped) remains a **future** build — **risk 4.3 / §5 P2, unbuilt**. This subsection records that the governing citation the future fence will cite now exists; it does not imply the fence exists.
- **Record:** `docs/briefs/results/ndis-sensitive-real-imagery-rule.md`; ratification packet `docs/briefs/ndis-sensitive-real-imagery-rule-ratification-packet-v1.md`.
...
### 3.6 Convergence review — external design input (2026-07-19, evaluated + reconciled)

An external design proposal (via PK) was evaluated against this design. It **converges** with the music-precedent model on the whole shared spine — shared storage + DB-authoritative eligibility, a first-class **vertical** scope (already §3.2b), fail-closed exclusions, client-overrides-generic, scoped approval, fenced-until-approved, and a multi-entity licence bar. Independent arrival at the same shape is a **confidence signal, not a redesign.** Three **additions** are folded in below (→ OQ-6, OQ-7); three **overrides** are recorded so they are not silently re-proposed later.

**Additions (→ OQ-6 / OQ-7):**
- **Per-client `asset_pool_policy`** — `client_only` (never fall back to shared; fail-closed / request-sourcing when the client pool is empty) · `client_preferred` (client first, shared fills gaps — today's implicit behaviour) · `best_fit` (score client + shared together with a client bias). Default `client_preferred`. Extends the existing `c.client_music_profile` per-client-preference precedent (§2.1) — a small column, not a subsystem. Closes a real gap: today's design cannot express `client_only` exclusivity and would **silently allow shared fallback**.
- **`purpose_bound` as a distinct no-auto-rotation class** (+ a richer `sensitivity_class` descriptor and a `cultural_review_required` flag). Stronger than "kept out of the shared pool": a consent-bound / participant / story- or campaign-bound asset must not enter **automatic rotation even within its own client** (today, if `is_active`+`approved` for its client, `resolve_slot_assets` rotation could pick it). Strictly strengthens **Rule 3.5** enforcement and gives the already-named First-Nations cultural sign-off requirement (D7 exit-test) a real field.
- **Pool-attribution in explainability** — extend the existing `selected/rejected/reason` contract (§1.6) to name the source pool ("selected from: ndis vertical-shared; N client-exclusive alternatives available"). Small, additive.

**Overrides (ICE constraint wins over the external proposal):**
- **No new bucket.** Keep `brand-assets` — the resolver output-as-input guard checks `bucket='brand-assets'`; a new `creative-assets/` bucket breaks it. Adopt the proposed `verticals/` sub-organisation as a **prefix inside `brand-assets`** (`_shared/verticals/<vertical>/`), not a new bucket. Folders stay organisational; the DB is authoritative (§3.1).
- **Keep scoped rows, not a flat enum / client arrays.** The music `scope_kind / scope_value / fit_status` rows (§3.2b) express simultaneous `suitable`/`blocked`/`needs_review` across scopes and are more auditable than a single `governance_scope` enum or `allowed_clients[]`/`excluded_clients[]` JSON arrays. Any `governance_scope` label is at most a **derived summary**, never the source of truth.
- **`production_use_allowed` is not a live fence today** (§1.2 — written by intake, read by no resolver). Treating it as an active eligibility gate is a **T3 resolver change** (add it to the eligibility chain), not a metadata toggle — do not assume it fences until that lane ships.
...
| 4.3 | **Sensitive-content exclusions.** Some assets must NEVER be generic — NDIS logo/affiliation cues, participant / culturally-specific / consent-bound imagery. | **High** | **Hard class exclusion at the pool boundary:** such assets stay client-scoped and are structurally ineligible for the shared table (`brand_neutral=false` + a policy fence). Grounded in **NDIS content Rule 3.5** (identifiable real-participant / person-with-disability imagery requires *verifiable consent* and stays client-scoped, structurally ineligible for the generic pool; `docs/compliance/ndis_content_rules.md` Rule Group 3 — the visual sibling of Rule 3.4) and the D7 purpose-binding (brand-navy abstract intaken; civic/real imagery routed back, not pooled). **OQ-1 RESOLVED (2026-07-19):** the seed's *"v5.79 NDIS Sensitive-Real-Imagery policy"* did not exist at HEAD (grep-confirmed present only in this packet); PK ratified a fresh **Rule 3.5** — analogy-grounded on Code of Conduct Rule 2 + Provider Guidance para 31-35 / para 32, NDIS-vertical-scoped — as the governing citation. Brand/affiliation-neutrality of pooled imagery stays with risk 4.1's `brand_neutral` mechanism (per OQ-D). Ratification packet: `docs/briefs/ndis-sensitive-real-imagery-rule-ratification-packet-v1.md`. |
...
- **OQ-6 (per-client asset-pool policy — external-input add, §3.6):** Adopt a per-client `asset_pool_policy` enum (`client_only` / `client_preferred` / `best_fit`, default `client_preferred`) on the per-client preference table (`c.client_music_profile` precedent)? It governs whether shared fallback is permitted at all and whether a superior shared asset may outrank a client asset. PK to decide the default and whether `client_only` (exclusivity) and `best_fit` (shared-can-win) are in v1 scope. Recommend: adopt, `client_preferred` default, `client_only` in v1 (real exclusivity need), `best_fit` deferred.
- **OQ-7 (purpose-bound / sensitivity class — external-input add, §3.6):** Adopt `purpose_bound` as a distinct no-auto-rotation fence + a `sensitivity_class` descriptor + a `cultural_review_required` flag? Strengthens Rule 3.5 (fences consent-bound / participant / cultural assets out of automatic rotation **even within their own client**) and operationalises the First-Nations cultural sign-off requirement (D7 exit-test). PK to decide inclusion and whether `purpose_bound` blocks automatic rotation only, or all non-explicit selection. Recommend: adopt; `purpose_bound` blocks automatic rotation (explicit operator/PK pick still allowed).
