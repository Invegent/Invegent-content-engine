# Post-CGU-v1 — Phase 2 Schedule Expansion — Apply Packet v4
...
**Supersedes v3** (hash `1ccaa373106244367e16485b5d68bb17b0b6f68184eb5b7f08b281b9f585b24a`),
**v2** (hash `2a1fec1a1e24b2c994d40dc7fc79357cb845a0409f5a5927f9fe8072aef561a6`), and **v1**
(hash `5e0cb969797b3816a8e478c0cfe9f986227e22eec9a54172459297ec40bb05eb`). None were edited
in place. v3's own edit (adding item 16 to fix v2's 2 findings) introduced 2 new §5
documentation-ordering findings on re-audit — both confirmed by `apply-harness-auditor`
as **documentation-only, zero functional SQL impact** ("every failure path in §3/§4
still fails closed and aborts the transaction regardless of the declared list's order or
completeness framing"). v4 fixes both, with **no SQL logic change from v3/v2** — the
`§3` transaction body is unchanged; only `§5`'s numbering/header and one inline SQL
comment are corrected:

1. **AHA-08-2 (medium, fixed):** v3's item 16 ("Total scope sanity") actually fires
   third-from-last in execution (inside §3 step 7, before the Protection B block begins)
   but was listed last, contradicting the header's own "in actual execution order"
   claim. It is now item 14, correctly positioned between the baseline re-checks (item
   13) and the two readiness checks (now items 15–16).
2. **AHA-01-3 (low, fixed):** v3 dropped v2's explanatory scope-disclaimer paragraph
   when the header was reworded for v3, partially reopening AHA-01-1's completeness
   over-claim. The disclaimer is restored below, updated for the corrected numbering.

## Note on repository state at freeze time

Between v3's freeze and this v4 freeze, an unrelated commit landed on `main` in this
shared worktree (`ebdb42a` → `1b66c1a`, "runtime: v6.130 — schedule-expansion apply
(v11) closed…", writing `docs/00_action_list.md`, `docs/00_sync_state.md`, and
`docs/briefs/results/post-cgu-v1-optimum-schedule-expansion-apply-result-v1.md`).
`branch-warden` confirmed this commit did **not** touch or collide with any Phase-2
packet file, and that v3's on-disk hash matched its pinned value exactly — no
corruption, no collision. It reads as this same session's own v11 closeout paperwork
landing concurrently, not a competing packet lineage. Flagged here per this lane's
standing disclosure practice; HEAD will be re-verified again immediately before any
future commit action on these packet files.

---

## 1. Exact row-level manifest (17 rows) — unchanged from v1–v3
...
## 2. Before / after volume matrix — unchanged from v1–v3
...
## 3. SQL — the single, atomic apply transaction (unchanged from v2/v3 — no logic edits)
...
BEGIN
  ...
  -- Total scope sanity (item 14 in §5 — deliberately redundant, ownership-agnostic
  -- double-check of the missing/extra check above, using a different query shape)
...
**Execution channel rule:** identical to §3's rule — this entire rollback script, from
`BEGIN;` through `COMMIT;`, MUST be submitted as the `query` parameter of exactly ONE
call to `mcp__supabase__execute_sql`. Never split across multiple calls.
...
## 5. STOP conditions — 16-item list, corrected to true execution order

This list rolls up Protection A's 12 individual client-resolution abort paths (4
clients × {zero-active-rows, multiple-active-rows, drift}) into item 1, and each round
of 9 individual baseline checks (PP total, PP carousel config, CFW LI image_quote, CFW
LI total, invegent LI, CFW YT, invegent YT, NDIS YT, NDIS LI) into items 9 and 13 — it
does not itemize each of those 21 sub-checks separately. Item 14 is a deliberately
redundant, ownership-agnostic double-check of item 11 (different query shape: a plain
`schedule_id IN (...)` existence count vs item 11's ownership-aware `EXISTS` join), kept
for defense-in-depth, not because it is a distinct safety property.
...
9. Any pre-apply baseline mismatch (9 sub-checks, listed above).
...
14. Total scope-sanity re-check on the 17 rows post-UPDATE (redundant double-check of #11, different query shape).
15. Any readiness cell regresses from ready to not-ready (zero tolerance this packet).
16. Any readiness cell becomes newly ready outside the 3 named expected appearances.
...
v4 hash, a fresh `branch-warden` HEAD re-check, then a hash-pinned `ask_chatgpt_review`
(mandatory — this packet is DML). Execution requires a separate, explicit PK
production-authorization message naming this exact file and its frozen hash, per this
lane's standing gate.
