...
**Status:** **DESIGN-OF-RECORD — RATIFIED 2026-07-19 (PK consolidated gate; OQ-2..OQ-7 accepted as proposed).** Implementation remains unbuilt — every step (P2 dark DDL → P5 resolver wire → P6 dashboard) is a separate future gate at its own tier. The R1 condition stands: `db-rls-auditor` must re-run MCP-enabled (+ `get_advisors`) before any P2 DDL.
