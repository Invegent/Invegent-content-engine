> ## 📌 LANDED ON MAIN 2026-07-24 — docs-only reconciliation (recording lane, register v6.14)
> **Provenance:** byte-exact copy of `docs/briefs/dashboard-privileged-action-containment-batch-2-brief-v1.md`
> from branch **`origin/claude/containment-batch-2-brief`**, commit **`a14dff9`**
> (2026-07-22T19:50:33+1000) — *"docs(security): Gate-1 brief — Dashboard Privileged-Action
> Containment Batch 2 (PK-approved, rev 2)"*. Verified **NOT an ancestor of `origin/main`**
> (`git merge-base --is-ancestor a14dff9 origin/main` → false).
> **The branch is NOT merged and is NOT retired by this landing** — retirement is a PK call.
> **Landed docs only.** No implementation code was landed. Batch 2 remains **NOT IMPLEMENTED** and has
> **NOT completed its own review chain** (§8: `security-auditor` → `branch-warden` → external review
> pinned to the brief hash → PK Gate 1). PK has answered D-1…D-4 (§4); implementation is still
> unauthorized.
> **⚠ Read with the companion boundary document** —
> `docs/briefs/dashboard-containment-batch-2-boundary-and-eq2-proof-v1.md` (session S1, 2026-07-24)
> reports a read-only census finding **six further caller-controllable `exec_sql` sites besides the two
> sinks scoped here, three of them GET-reachable**, so **Batch 2 would close two of eight**. That
> document is itself **DRAFT, awaiting PK**. **Nothing in this brief's body has been altered.**
> Register pointer: **v6.14**.
