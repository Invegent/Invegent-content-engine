...
| PK apply gate | **STOPPED — awaiting PK** | Nothing applied, deployed, migrated, or committed to main by this lane. **Remaining prerequisites to the apply gate (PK sitting 2026-08-01, item 3):** packet §8 Q1–Q5 PK decisions · the two named pre-apply `execute_sql` checks (all-NULL constraints baseline re-verify + live PostgREST exposed-schema list) · then the SEPARATE PK apply authorisation against this exact hash. |
