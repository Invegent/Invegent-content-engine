...
`Complete` — build → merge → push done, **§9 backtest PASSED**, status **PROVEN** (2026-07-19). The deploy-content classifier (checks 1–3) passed the manual blind backtest **and** the native registered-agent re-run under a corrected **two-verdict** contract; drift is advisory; deploy-verifier is now listed in the CLAUDE.md team table. Deploy remains the PK hard stop; the agent confirms, never acts.
...
**Verdict:** `Pass (PROVEN)` — build/merge/push verified clean (agent `a52e788` on origin; v5.86 register record `82ec2aa` on origin). **§9 backtest PASSED (manual smoke + native re-run)** at a PK gate (2026-07-19). A manual blind backtest ran the charter on three scenarios against independent ground truth on live `image-worker`:
...
The backtest **caught one design finding**: check 4 (drift) is unreadable read-only (no credential-free path; the Governor must never handle an `x-series-key`), and under the original single-verdict rule that forced `overall=MISMATCH` on *every* good deploy → a STOP-on-every-good-deploy that erodes trust. **PK ruling (2026-07-19):** split the output into an independent `deploy_content_verdict` (checks 1–3, a content MISMATCH is a hard STOP) and an advisory `drift_verdict` (CLEAN / DRIFT_DETECTED / REFRESH_NEEDED / UNREADABLE; drift unreadable → FLAG, never a content STOP), with `overall` gaining `PASS_WITH_FLAG`. The charter was corrected as a surgical T1 edit and the three scenarios **re-ran clean under the new contract** (A→`PASS_WITH_FLAG`, B→`MISMATCH`, C→`MISMATCH`; drift never set `human_stop_signal` or downgraded content). **Native registered-agent re-run PASSED** (2026-07-19) — the same three blind scenarios run through the **native registered `deploy-verifier` subagent** (real read-only tool-fencing), results identical to the smoke: A→`deploy_content_verdict=PASS`/`overall=PASS_WITH_FLAG`, B→content `MISMATCH` (marker-absence dominated the version decoy), C→content `MISMATCH` (verify_jwt); zero false content-MISMATCH, `source_read` named every time, drift `UNREADABLE`→FLAG never a STOP. **→ PROMOTED to fully PROVEN**; deploy-verifier is now in the CLAUDE.md team table. (The agent-type became invocable once the charter landed on origin — no fresh session was needed.)
