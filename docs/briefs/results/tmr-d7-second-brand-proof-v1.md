...
- **(c) background intake+promotion — ✅ ENVELOPE STAGED** (`docs/briefs/tmr-d7-c-ndis-background-intake-envelope-v1.md`). Asset pinned: local final `ndisyarns_bg_brand_texture_v1.jpg`, bytes 57991, sha256 `40fb69b42a881533e9f1bb73380330073f6f89e3a2150cabf7ca045ab7b788f5`, 1920×2877 JPEG. NOTE: the "final" is byte-identical to the source download (uncropped portrait) — consistent with PP (template Background covers/crops to 1:1); pre-crop optional (TMR-supp-ndis-bg).
- Two PK-gated applies: (1) fenced intake — upload to `brand-assets/NDIS_Yarns/Backgrounds/bg_ny_brand_texture_navy_waves.jpg` + fenced INSERT + guards (byte-verify · public-URL sha256 · in-txn pool-neutrality · db-rls-auditor); (2) promotion — un-fence to eligible after PK crop-proof/visual (sets `safe_for_text_overlay`). Surface DISJOINT from N7b (storage/DB vs worker code) → parallel-safe.
