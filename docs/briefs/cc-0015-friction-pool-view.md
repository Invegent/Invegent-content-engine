...
**Status: AUTHORED + V-G1 APPROVED (v3.13.3, 2026-05-26).** `docs/process/ICE-PROC-002-pooled-resolution.md` written (documentation-only; 0 production mutation). Covers all 7 required sections + COEXIST model + current saved views + known deferrals; linked from `docs/00_docs_index.md` (V-G3). Scoped to the shipped surface only — does NOT assume Stage C/D/E exist; does NOT claim cc-0015 complete. **V-G1 PK-approved 2026-05-26** — ICE-PROC-002 is now the operating reference for the current cc-0015 `/operations` saved-views + pooled-resolution surface. All Stage G V-checks (V-G1 approval / V-G2 committed / V-G3 docs-index link) now satisfied. The +4-week process review (≈ 2026-06-26) remains the next ICE-PROC-002 milestone.
