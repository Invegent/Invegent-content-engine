...
**Status: AUTHORED (v3.13.2, 2026-05-26).** `docs/process/ICE-PROC-002-pooled-resolution.md` written (documentation-only; 0 production mutation). Covers all 7 required sections + COEXIST model + current saved views + known deferrals; linked from `docs/00_docs_index.md` (V-G3). Scoped to the shipped surface only — does NOT assume Stage C/D/E exist; does NOT claim cc-0015 complete. V-G1 (PK draft approval) is the remaining V-check.
