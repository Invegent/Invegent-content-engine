...
**Stage B (RPC) APPLIED 2026-05-26 (COEXIST):** `friction.fn_recent_cases` extended with backward-compatible `p_categories`/`p_triage_states`/`p_action_decisions` filters (migration `cc_0015_b_fn_recent_cases_filters` v`20260526124005`; plan_review D-01 `58d89efe` + sql_destructive D-01 `17db8b27` + PK phrase; file backfilled to `supabase/migrations/`). Verified: V-B2 byte-identical no-filter equivalence (`fn_recent_cases(50)` == `(50,NULL,NULL,NULL)` == baseline n=44/md5 `af67c0ee…`); filters restrict with 0 leakage; single fn (no overload); return shape unchanged. **Stage B FRONTEND** (filter bar + saved views + category-description hydration on `/operations`) = **separate next gate** (build → ef_deploy + merge, each PK-gated). Source filter + sort deferred; `/operations/pools` v0 untouched; Stage C/D/E excluded. **cc-0015 still NOT complete.** **DEVIATION — category per-option descriptions RE-SCOPED to a Stage F follow-up (PK decision 2026-05-26):** current wiring surfaces the category field guidance + the `unclassified` warning; live per-option `friction.category.description` display is **deferred** (needs DB-backed category hydration) — does NOT block visual verification of the core Stage F help system. FAB has no `capture_reason` field (field absent, not omitted help). **Stage B/C/D HELD pending a re-brief** reconciling this category-based pool view vs the shipped notes-based Pools/Repair-Board v0 (`/operations/pools`, commit `9a4b446`) — v0 NOT superseded. Stage A.5 backfill recategorisation = separate manual PK-confirmed step. *(Brief drift: line 10 "one new column on friction.case / no new tables" contradicts §3 which adds the `pool_session` table and no column — §3 authoritative. Stage F dict lives at `lib/friction-help-copy.ts` not `app/lib/` per repo `@/*`→root convention.)*
