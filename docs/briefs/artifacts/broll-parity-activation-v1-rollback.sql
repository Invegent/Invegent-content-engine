-- ============================================================================================
-- ROLLBACK — B-roll PARITY Activation v1
-- Restores the EXACT pre-apply values of fit_status / fit_reason / reviewed_by / reviewed_at on all
-- 3 rows, returning PP governed video_short_stat selection to incumbent a3d8472d (1080x1920/12s).
-- (`updated_at` is deliberately set to now(), NOT restored — nothing reads it; see packet.)
--
-- LINEAGE + LIVE VALIDITY: byte-identical in its executable body to
-- `_harness/cc_broll_activation_20260728/rollback.sql` REV 3, which PERFORMED THE REAL v6.48 ROLLBACK
-- on 2026-07-28 and proved itself digest-exact. Its target digest 962043fb55bdd2a1a9e5d0a8718118e0 was
-- recomputed against live production on 2026-07-29 and MATCHES, so the literals below still reproduce
-- the true current pre-image. Proven again in-transaction this lane: see rollback_proof.sql /
-- rollback_proof_output.txt (forward + rollback composed in ONE transaction ending in ROLLBACK).
--
-- NOTE ON SCOPE: this reverses the SELECTOR repoint only. video-worker v3.15.0 does NOT need to be
-- rolled back with it — its parity overlay is keyed to template 46c5c4ac, which stops being selected
-- the moment this file commits, so v3.15.0 becomes inert rather than wrong. Reverting the EF is a
-- separate, optional step and is NOT required to restore the incumbent's output.
--
...
--   (2)  atomicity guard re-asserted (detective) + DIGEST-EXACT restore to the frozen pre-image
--        + winner must be back to a3d8472d                                    else ABORT
