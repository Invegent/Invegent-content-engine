-- LIVE BASELINE — m.fill_pending_slots(integer) — pg_get_functiondef fresh read
-- Read via: SELECT pg_get_functiondef(p.oid) FROM pg_proc p JOIN pg_namespace n
--   ON n.oid=p.pronamespace WHERE n.nspname='m' AND p.proname='fill_pending_slots'
-- Read at: 2026-08-06 (control-tower orchestrator, R0 catalog path via db-read.py).
-- Project: mbkmaxqhsohbtwsqolns. 806-line live function — this IS the ground truth
-- (do not use the 20260729143000_s9_layer1_capability_gate_fill_pending_slots.sql
-- migration file's copy without diffing it against this fresh read first — the
-- migration ledger can drift from git per the standing repo gotcha).
--
-- OPTION-B FIX TARGET (S1 diagnosis contributing-defect-A): TWO sites both read
--   `sp.fitness_score_max` (class-wide MAX across all formats) instead of the
--   format-specific number `sp.fitness_per_format->>v_chosen_format` that
--   check_pool_health()'s own code comment says should be used:
--     1. main candidate_pool CTE, line ~422 below:
--        (sp.fitness_score_max * COALESCE(rpc.fitness_multiplier, 1.0)) AS effective_fitness
--     2. relaxed_pool CTE (the post-relax retry path), line ~509 below:
--        (sp.fitness_score_max * COALESCE(rpc.fitness_multiplier, 1.0)) AS eff_fit
--   BOTH must change together — fixing only one would make the relax retry compute
--   a DIFFERENT effective_fitness basis than the initial pass, which is incoherent.
--   S1 verified empirically that swapping to the per-format number does NOT change
--   today's CFW outcome (link 1/2 — body-health + decay — dominate either way).
--   fitness_per_format is a jsonb column keyed by ice_format_key on m.signal_pool
--   (confirmed live, see the check_pool_health baseline file in this same
--   directory) — extraction is `(sp.fitness_per_format->>v_chosen_format)::numeric`,
--   COALESCE to sp.fitness_score_max if the per-format key is absent for a given
--   row (fail-open to the CURRENT behavior on missing data, never silently drop
--   a candidate that simply lacks a per-format breakdown yet).
--
-- OPTION-C DEPENDENCY: this function calls m.check_pool_health() at line ~488
--   (`IF v_pool_count < v_quality.min_pool_size_for_format THEN SELECT
--   m.check_pool_health(...) INTO v_pool_health; IF (v_pool_health->>'health') =
--   'red' THEN ... relax ...`). Option C changes check_pool_health()'s OWN body
--   (separate artifact); this function's call site and 'red'-string comparison
--   are UNCHANGED by Option C — the fix is entirely inside check_pool_health(),
--   so fill_pending_slots does not need to change for Option C at all. Confirm
--   this in the NOT_APPLIED artifact: Option C touches ONLY check_pool_health();
--   Option B touches ONLY fill_pending_slots' two effective_fitness expressions.
--   Keep them as two clearly labeled sections in the same or separate migration
--   file, but do not conflate the two functions' bodies.
