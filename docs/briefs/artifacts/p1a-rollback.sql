-- Rollback: p1a_schedule_format_override_surface
-- Reverses migration 20260727100000_p1a_schedule_format_override_surface.sql.
--
-- ORDER (as written below):
--   (i)   CREATE OR REPLACE public.get_week_format_allocation back to the
--         byte-exact live base (md5 11e566b52347dd396f04b481e299bb7c). The base
--         body does NOT reference format_override.
--   (ii)  DROP FUNCTION public.save_week_format_override(uuid, jsonb).
--   (iii) ALTER TABLE c.client_publish_schedule DROP COLUMN format_override.
--
-- !! DEPENDENCY: If migration 1c (20260727100100_p1c_materialise_slots_honour_
--    format_override) is ALSO applied, its m.materialise_slots body references
--    cps.format_override. Roll 1c back FIRST (apply p1c-rollback.sql, which
--    restores m.materialise_slots to a base body with no format_override
--    reference) BEFORE running step (iii) here — otherwise the column drop
--    leaves a live function referencing a dropped column.
-- ============================================================================

-- (i) Restore get_week_format_allocation to the byte-exact live base ---------
...
-- (ii) Drop the save RPC ----------------------------------------------------
DROP FUNCTION IF EXISTS public.save_week_format_override(uuid, jsonb);

-- (iii) Drop the override column --------------------------------------------
ALTER TABLE c.client_publish_schedule DROP COLUMN IF EXISTS format_override;
