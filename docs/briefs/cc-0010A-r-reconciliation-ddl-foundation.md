...
**Last patched:** 2026-05-12 Sydney (v1.5 — V6c row-count assertion tightened from >= 86 to = 86 per CCD v1.4 narrow review)
**Author:** chat (Claude)
...
- **2026-05-12 Sydney — v1.5 — V6c row-count assertion tightening from CCD narrow review.** CCD completed narrow re-review of cc-0010A v1.4 at commit `2035a3a8` and returned verdict *agree-with-corrections, risk low, no blocking corrections, commit accepted, cc-0010A may re-run live pre-flight + D-01.* The sole non-blocking suggestion was to tighten the V6c row-count assertion from `total_r_column_rows >= 86` to `total_r_column_rows = 86`. Rationale (CCD): the §3.7 contract is exactly 86 rows; the V6c filter is bounded to the six cc-0010A tables and excludes the two cc-0009 r.* tables by `tr.table_name IN (...)` clause, so the only reasons total could exceed 86 are JOIN duplication, trigger-stub inflation, or unexpected DDL drift — all of which the strict equality should fail loudly on. v1.5 applies this single-line tightening only. No other surface changes; §3.7 Fix A pattern preserved verbatim; v1.3 `queue_id` FK target correction preserved verbatim at all 5 loci; §1.7b probe preserved verbatim; all other V-checks preserved verbatim. Parent cc-0010 brief NOT patched. No production mutation during this patch — doc-only.
...
- `total_r_column_rows = 86` (strict equality — §3.7 contract is exactly 86 rows; the V6c filter is bounded to the six cc-0010A tables which are distinct from the 2 cc-0009 r.* tables; strict equality catches JOIN duplication, trigger-stub inflation, and unexpected DDL drift per CCD v1.4 narrow review)
