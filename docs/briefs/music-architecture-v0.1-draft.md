...
### RULED OUT: platform-native sound libraries (YouTube Shorts / Instagram Reels)

PK's thought: YouTube and Instagram let creators pick from an in-app sound library for Shorts/Reels for free — could ICE just draw from those instead of a paid vendor? Checked live 2026-08-06 (web search, not primary-source terms pages — lower confidence than the ElevenLabs/Epidemic Sound reads above, but consistent across independent sources):

- **YouTube Shorts audio library**: licensed only for Shorts under 60 seconds, added *through YouTube's own in-app picker*. Not downloadable or usable outside that specific context — the same track in a long-form video or another platform triggers a Content ID claim.
- **Instagram Reels music library**: licensed for *personal, non-commercial* use. Business accounts (what ICE's brand pages are) don't get the full library at all — they're restricted to a smaller "Meta Sound Collection." Reuse of a downloaded track on another platform is explicitly not covered.

**Ruled out on architecture grounds, not just licensing:** even where the license is real, it's scoped per-platform, per-format, and to that platform's own native app picker. ICE's pipeline renders one MP4 with audio baked in (`video-worker` + Creatomate) and publishes that same file to YouTube, Instagram, Facebook, and LinkedIn (S9 publisher enforcement design — see [[s9-publisher-enforcement-live]]). A platform-native sound library can't feed a render-once/publish-everywhere pipeline no matter how permissive its in-app terms are — there's no path from "pick a Shorts sound" to "bake it into a server-rendered video also going out on three other platforms." Not re-opened unless the publish architecture itself changes to render per-platform natively (not currently planned).

### Status after this addendum

No vendor decision made. No signup, trial, or Enterprise-quote request initiated for either vendor. PK's instruction this session: **continue on the current course; revisit this lane later.** The five "Required confirmations" in the original §[Required confirmations before implementation] section are unchanged and still open — this addendum adds a sixth, informal one: *Epidemic Sound's "no standalone use / no third-party AI tool" clause as applied to an automated selection pipeline, unconfirmed.* This does not retire or supersede the Stage-0 prompt-sandbox plan (Mubert/Stable Audio/Beatoven) — Epidemic Sound is an additional candidate for the *licensed-catalog* path (curated stock library), not the *generative* path those three represent; the two are different mechanisms toward the same music-bed goal and both remain open. Platform-native sound libraries (YouTube Shorts / Instagram Reels) are ruled out per above.
