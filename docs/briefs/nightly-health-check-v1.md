...
risk_tier: 0
owner: cowork
created_by: PK + chat session 2026-05-02 Saturday afternoon Sydney (v1 first-run learnings + B investigation patches)
...
Execute all 13 queries via Supabase MCP `execute_sql` against project `mbkmaxqhsohbtwsqolns`.
...
### Q-true-stuck (NEW v2) — Genuinely-stuck items (actionable)
...
- Next: PK reviews; if Section 10 has Priority 1 items, action; if pattern stable across 2-3 runs, schedule via Cowork at 02:00 AEST
```
...
## Success criteria for v2 first run

| Metric | Good | Re-evaluate |
|---|---|---|
| Questions asked | ≤ 3 | > 8 |
| Defaults overridden | ≤ 10% | > 30% |
| Schema bugs (v2 specifically should fix the v1 ones) | 0 | > 0 |
| Output file produced | yes | no |
| Production writes | 0 (mandatory) | any > 0 |
| PK review time | ≤ 3 min | > 10 min |
...
If 6+ "Good" thresholds hit AND zero schema bugs: brief shape locked. Schedule at 02:00 AEST after one more clean run.
...
If this brief is unchanged at 2026-06-02, evaluate: (a) is the data useful? (b) are there persistent questions that should become brief-spec'd? (c) ready to schedule daily without first-run-style oversight?
