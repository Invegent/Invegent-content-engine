...
**Unit B-2 prep (Strategy B) — PREPARED, NOT deployed/applied (2026-05-26):** `youtube-publisher` **v1.9.0** authored (approval predicate broadened `approved`→`IN ('approved','published')`; successful-publish path + all guards byte-unchanged) + bounded B2 recovery DML prepared as a non-executed artifact (`docs/briefs/results/yt-publisher-failed-no-retry-unit-b2-recovery.sql`). See §Unit B-2 (Strategy B) below. *(prepare step — was not deployed/applied at the time.)*
**Unit B-2 EXECUTED 2026-05-26 (v3.10):** v1.9.0 **DEPLOYED + verified live** (ef_deploy D-01 `4719f53b`); **B2 17-row recovery APPLIED + verified** (sql_destructive D-01 `2135ae11` + PK approval phrase; migration `yt_failed_backlog_recovery_unit_b2_token_casualty_20260526`; 17 `failed→generated`, `approval_status` preserved=`published`, draining ≤2/tick — 2 published so far). **Full Unit B (B1+B2) COMPLETE; B3 = onboarding debt.** ⚠️ **NEW finding F-YT-PUB-PUBLISH-AUDIT-GAP:** cross-posted YouTube publishes are marked `published` with a real `youtube_video_id` but get NO `m.post_publish` audit row — `uq_publish_attempt UNIQUE(post_draft_id, attempt_no)` + the publisher's hardcoded `attempt_no=1` collides with the draft's existing Facebook row, and the `insertErr` is swallowed (index.ts:258). Open follow-on (fix + backfill). See session `docs/runtime/sessions/2026-05-26-v3.10-unit-b2-deploy-recovery.md`.
...
**Order:** deploy v1.9.0 (Unit A of B-2) → then apply the DML (Unit B of B-2). Each gated on its own D-01 + PK approval phrase. **✅ EXECUTED v3.10 (2026-05-26): v1.9.0 deployed (ef_deploy `4719f53b`) → B2 DML applied (sql_destructive `2135ae11` + PK phrase); full Unit B (B1+B2) COMPLETE.** B3 (9 never-connected) remains onboarding debt, out of scope. *(New open item from execution: F-YT-PUB-PUBLISH-AUDIT-GAP — see Status block.)*
