...
- **v1.2** (2026-05-20 Sydney) — **Stage C APPLIED v2.98.** Migration `cc_0016_c_emit_manual_event_with_attachments` applied via `apply_migration` MCP after D-01 approval at `m.chatgpt_review` id `358c6fdd-fce3-4150-a780-9e4397fd2fc6` (verdict=agree, action_taken=proceed, risk_level=medium, requires_pk_escalation=false). 8 approved mutations applied atomically. `friction.emit_event` 13-arg live; `friction.fn_emit_manual_event` 8-arg live; old 12-arg + 6-arg overloads removed. cc-0017b unified emit pipeline preserved byte-stable. Grants: authenticated + service_role only; anon + public denied. **V-C0/V-C1/V-C2/V-C3a-e/V-C4/V-C5/V-C6 PASS** (11/11 SQL V-checks); V-A5 carried to Stage B FAB ship. 3 D-01 review rows close-the-loop resolved (`56e65bb2` + `dbabb576` escalated → resolved; `358c6fdd` completed → annotated; all `resolved_by='cc-0016-stage-c-apply-v2.98'`). Outstanding close-the-loop count 28 → 26. Brief structurally unchanged from v1.1 — v1.2 changelog records the apply event only. **Stage B (dashboard FAB upload/read UX) is the next gating action**; Stage B authorization constraint carried (application layer must verify authenticated operator is allowed to attach evidence before invoking `fn_emit_manual_event` with non-empty `p_attachments`).
