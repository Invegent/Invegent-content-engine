...
> **Status:** Stage 0–4B **LIVE IN PROD** (`mbkmaxqhsohbtwsqolns`; merge `49a6c0e` to `main`; migrations `20260527114041` tables + `20260527114144` 4 `public.*` SECURITY DEFINER RPCs + `20260527114333` security hotfix; dashboard Stage 4-A/4-B routes live at `dashboard.invegent.com/system/subscriptions`; RPC EXECUTE = service_role + postgres only). Repo↔prod migration-history reconciliation v2 merged on `main` via PR #3 (`d7543078`). **Stage 5 execution plan AUTHORED 2026-05-28** at [`docs/briefs/cc-0020/stage-5-live-gmail-execution.md`](cc-0020/stage-5-live-gmail-execution.md) — execution remains future/gated per-unit. **Unit B code-readiness MERGED on `main` 2026-05-29** via PR #6 (merge `9e4d3db`; head `1888b56`; two-commit progression `75cfba3` original + `1888b56` V-B7 RPC-only fix; 41 hermetic tests PASS; CCH plan_review CLEAN after V-B7; **function NOT deployed, NO runtime effect** — `SUBSCRIPTION_GMAIL_INGEST_ENABLED=false` default short-circuits). **Unit C BLOCKED** until `public.ingest_subscription_import_candidate(...)` write RPC migration is authored + applied behind `sql_destructive` D-01 + PK exact approval + real-PG/PGlite proof + `ef_deploy` U-C gate fires. **Unit A required** before U-D first smoke (OAuth Production status + `gmail.readonly` scope only + Supabase secrets set + name-only presence verified). **Unit D** blocked until U-A + U-C. **Unit E cron** = separate later gate. **Unit F Pub/Sub** = DEFERRED to Stage 5b. **Governance:** G1 (`m.chatgpt_review.6d9a4bd3` — migrations `114041`+`114144` plan_review) **RESOLVED 2026-05-28** (close-loop record by CCH). G2 (`20260527114333` hotfix governance row) **ACCEPTED-AS-APPLIED 2026-05-28** (PK directive, docs-only): the migration was a pure grant-tightening hotfix (REVOKE EXECUTE FROM `anon`,`authenticated` on the 4 cc-0020 RPCs) — no data effect, no schema change; live posture already verified correct (RPC EXECUTE = `service_role` + `postgres` only; `anon`/`authenticated`/`PUBLIC` denied); residual risk = none known; process gap recorded. **Not precedent for skipping D-01 on future grant changes.**
