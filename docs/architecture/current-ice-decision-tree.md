...
| **verified_against** | project `mbkmaxqhsohbtwsqolns`; ai-worker `8204a5c7`; heygen-worker `9acb17e8`; live `cron.job` read 2026-06-11 (69 jobs); live `pg_proc` source reads 2026-06-11 (incl. `materialise_slots` full source); DB migration registry `20260422074318`/`20260422084148`/`20260425113754`/`20260426–27*`; evidence audits `6fd5f4d`, `b74c8e50`, `e39fa3a9` |
...
| Initial format preference | Config (FB/IG/LI columns — only FB populated) / **Hardcoded (YT)** | `m.materialise_slots` **reads `preferred_format_{facebook,instagram,linkedin}` from `c.client_publish_profile` for all three social platforms** — FB holds 'image_quote', **IG/LI columns are null, so the IG/LI null behaviour is config data, not absent code**; **`video_short_avatar` hardcoded in function source for YouTube (the A2 override)** | `m.slot.format_preference` (text[] — only element [1] ever populated) | A2's true home is slot creation, not ai-worker |
...
**Current status: dormant/orphaned policy subsystem — structurally valid, not live.** Zero current callers, no cron entry; the functions still compile against extant tables. Actual live output is a near-monoculture — FB/IG ≈ all image_quote, YouTube 100% avatar — produced by the FB preference column, the YT hardcode (which inverts the seeded design: avatar was specified at 10% of YouTube output, the hardcode makes it 100%), the null IG/LI preference columns falling through to the COALESCE default, and advisor overrides. Do not read aspirational config as live behaviour.
...
| 2026-06-11 | CCH | Option C replay audit: `materialise_slots` full source read; `m.slot.schedule_id` FK → `c.client_publish_schedule` (pg_constraint); `match_demand_to_canonicals` source head | §7: dormant subsystem list extended with `c.client_match_weights` + `c.client_dedup_policy`. §3 precision: `materialise_slots` reads `preferred_format_*` for all three social platforms — IG/LI null is config data, not absent code. Live chain and decision fork unchanged |
