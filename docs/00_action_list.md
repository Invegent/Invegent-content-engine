...
>
> Last updated: 2026-05-13 Sydney (**v2.71 — cc-0012 CLOSED-WITH-VERIFIED-VARIANCE.** Platform Reconciliation View (PRV) v1 delivered as DDL-only build. Stage A migration `cc_0012_op_platform_reconciliation_view` recorded at version `20260513094128`; `op` schema + `op_reader` role (NOLOGIN) + 5 plain views (`op.v_reconciliation_summary`, `op.v_per_client_rollup`, `op.v_per_platform_rollup`, `op.v_drift_rollup`, `op.v_freshness_rollup`) live. GRANT/REVOKE discipline preserves operator-only exposure (SELECT to `op_reader` + `service_role` only; REVOKE ALL from `PUBLIC` + `anon` + `authenticated`; zero PostgREST exposure). Stage B V1–V10 all PASS. V9 anti-write invariance verified: zero cc-0012-attributable writes to r.* / m.* / c.* surfaces. **Reconciliation v1 + PRV v1 family complete end-to-end** (cc-0009 + cc-0010A + cc-0010B + cc-0010C + cc-0011 + cc-0012 all closed). 0 chat-fired D-01s this session (PK-direct directive cadence — same operational mode as cc-0010C v2.69, cc-0011 v2.70). L45 truth: **Var-A1** brief §6.1.3 `information_schema.columns` primitive excludes matviews; **Var-A2** brief §7 V5 narrative 7-client vs actual 4-client source-derived; **Var-A3** `op.v_freshness_rollup.attention_needed` NULL under LEFT JOIN + SQL 3VL with empty `r.platform_observer_health`. All three carry-only — no silent resolution. **Candidate L57 NEW** = relkind-aware primitive selection (`pg_attribute + pg_class.relkind IN ('r','m','v')` rather than `information_schema.columns` for column-shape probes targeting potential view/matview surfaces); companion to L55 + L56. **Next major:** cc-0013 Dashboard Phase 0 (consumes cc-0012 operator views) + v1.6 cc-0010A doc patch (3 items, unchanged carry) + v1.3 cc-0011 minor doc patch (5 carry items: E1 + Var-A + Var-B + Var-C + Var-E) + cc-0012 v1.1 minor doc patch (3 new carry items: Var-A1 + Var-A2 + Var-A3) + close-the-loop batch sweeps.)
...
> **Last rebuilt:** 2026-05-13 Sydney (v2.71).
> **v2.71 note:** cc-0012 Platform Reconciliation View (PRV) v1 CLOSED-WITH-VERIFIED-VARIANCE. **Reconciliation v1 + PRV v1 family complete end-to-end** (cc-0009 + cc-0010A + cc-0010B + cc-0010C + cc-0011 + cc-0012 all closed). cc-0013 Dashboard Phase 0 promoted rank 1 (NEW; natural successor; consumes cc-0012 operator views). v1.1 cc-0012 minor doc patch rank 2 (NEW; 3 carry items: Var-A1 + Var-A2 + Var-A3). v1.6 cc-0010A doc patch rank 3 (3 items, unchanged carry from v2.68). v1.3 cc-0011 minor doc patch rank 4 (5 items unchanged carry from v2.70). Close-the-loop batches rank 5 (12 sessions overdue v2.71). F-K-SCHEMA + L34 audit rank 6. Personal businesses standing.

| Rank | Item | Priority | Why now | Next action |
|---|---|---|---|---|
| 1 | **cc-0013 Dashboard Phase 0** | **P1 (NEW rank 1 v2.71)** | Natural successor to cc-0012 closure. PRV v1 operator-views layer now live (`op.v_reconciliation_summary` + 4 rollup views); dashboard is the rendering layer that consumes them. cc-0012 v1.1 close declaration explicitly named cc-0013 as the next major item. Scope-definition D-01 to fire next per directive ("After this: Fire cc-0013 Dashboard Phase 0 scope-definition D-01"). Should apply v2.71 lessons: L57 candidate (relkind-aware primitive selection for any schema-shape probe) + carried v2.70 candidates L55 + L56. Operator path: assume `op_reader` from postgres / service_role (NOLOGIN binding deferred to cc-0012 v2; dashboard architecture should accommodate this). | PK → chat | PK directive on scope; brief authoring at `docs/briefs/cc-0013-{slug}.md`. |
| 2 | **v1.1 cc-0012 minor doc patch (NEW v2.71; 3 carry items)** | P2 (NEW v2.71) | Fold L45 close carries into cc-0012 brief: (a) **Var-A1** — switch brief §6.1.3 pre-flight column-shape probe from `information_schema.columns` to `pg_attribute + pg_class.relkind IN ('r','m','v')` (matview-aware); (b) **Var-A2** — align §7 V5 narrative with source-derived 4-client baseline (was stated as 7); (c) **Var-A3** — apply `COALESCE(observer_is_healthy, true)` (or equivalent NULL-safe rewrite) in `op.v_freshness_rollup.attention_needed` expression. May include `CREATE OR REPLACE VIEW op.v_freshness_rollup` to fix Var-A3 if PK greenlights minor production view-replacement at the same patch turn. No schema delta. | chat → future session | Doc-only patch via local git (or doc + view-replacement if PK directs). |
| 3 | **v1.6 doc-only patch to cc-0010A (3 items, carry from v2.68)** | P2 | Fold disclosures into brief: (a) `out` → `result_jsonb` rename in §2.7 + PRV-0 §4.3 update (carry from v2.67); (b) `r.set_updated_at` trigger audit (cc-0010A v1.5 E1: trigger not bound to r.ice_publication_evidence + r.reconciliation_run despite r.* registry membership); (c) `m.post_publish.queue_id` non-FK semantics documentation for future brief authors (cc-0010B F4 → L53 candidate). No production mutation. | chat → future session | Doc-only patch via local git. |
| 4 | **v1.3 cc-0011 minor doc patch (5 carry items, unchanged from v2.70)** | P2 | Fold L45 close carries into brief: (a) E1 cadence_anomaly trigger/severity wording reconciliation (§3.1 internal inconsistency); (b) Var-A late-state UPDATE deferral documented (§3.2 → v2 amendment); (c) Var-B MV refresh start-of-run codified (§3.3 → v1 ordering); (d) Var-C §2.1 stray `evidence_at` reference cleanup; (e) Var-E `classifyExpectedRow` date-parsing observational note. Also brief §5.1.1 column-name shorthand correction (`late_tolerance_minutes` → `minutes_late_tolerance`) to match Var-D source patch. No production mutation. | chat → future session | Doc-only patch via local git. |
| 5 | **Close-the-loop batch sweep (5-row + 24-row eligible)** | P2 (5-row UNBLOCKED 12 sessions overdue v2.71; 24-row historical eligible) | 5 prior cc-NNNN rows still in escalated status. 24 unrelated historical escalated rows untouched per CCH directive at v2.67–v2.71 — eligible for review/sweep next session. | chat → next session | Single execute_sql UPDATE with CASE expression for 5-row batch; separate review for 24-row historical batch. |
...
**Passive observation v2.71**: Cron 83 ice_evidence_materialiser_30min (every 30 min UTC; steady-state); cron 84 reconciliation_matcher_30min (every :15/:45 UTC; steady-state); cron 85 cadence_drift_checker_weekly (every Sunday 17:30 UTC; first natural fire expected 2026-05-17 17:30 UTC = Monday 2026-05-18 03:30 Sydney AEST). PRV v1 operator views now queryable via `SET ROLE op_reader` from postgres/service_role for ad-hoc sustained-health observation pending future Dashboard Phase 0 (cc-0013) rendering layer.
