...
**Status: Unit A IMPLEMENTED (v3.06 correction); Unit B NOT implemented.** Live DB has `m.is_publish_eligible(uuid,text)` + the `m.fill_pending_slots` gate (migration `20260524091020`, repo-backfilled `8738c3a0`; governed D-01 `a2e5f3f5` + PK approval per migration header). Unit B (ai-worker preflight) verified ABSENT in deployed ai-worker v2.12.0 — not implemented. v3.05's "brief only, NOT implemented" was stale.
...
| Status | Unit A IMPLEMENTED (`is_publish_eligible` created + `fill_pending_slots` gate live; migration `20260524091020`). Unit B (ai-worker preflight) NOT implemented (absent in ai-worker v2.12.0). |
...
| **F-YT-FAILED-NO-RETRY** | youtube-publisher v1.6.0 catch sets transient auth failure to terminal `video_status='failed'` (never re-selected) → token outage silently freezes rendered backlog | **P2** | **BRIEF AUTHORED v3.06** (`docs/briefs/yt-publisher-failed-no-retry.md`, `bee872aa`) | chat → PK | plan_review D-01 next; then ef_deploy + sql_destructive (Unit B = bounded recovery of 30 failed drafts) + PK approval. |
...
**Closed v3.06:** Deployed↔repo drift CLOSED (IG publisher `bc78511e`; 7 migration backfills `8738c3a0`). cc-0019 CORRECTED → Unit A implemented / Unit B not. IG-arc recording debt discharged. F-YT-FAILED-NO-RETRY brief authored (`bee872aa`). 0 production mutation / 0 D-01.
...
- **F-YT-FAILED-NO-RETRY brief** authored (`bee872aa`); plan_review D-01 next.
...
- 0 production mutation / 0 D-01. No decisions.md change. Dashboard PHASES leg N/A.
...
- **v3.06 (2026-05-25): Deployed↔repo drift close (repo reconciliation only — 0 production mutation, 0 D-01). IG publisher v2.0.0→v2.4.0 synced (`bc78511e`, byte-verified — rollback hazard closed). 7 chat-applied 2026-05-24 migrations backfilled to `supabase/migrations/` (`8738c3a0`, recovered verbatim from `schema_migrations.statements`) — DB→repo migration drift CLOSED. F-YT-FAILED-NO-RETRY brief authored (`docs/briefs/yt-publisher-failed-no-retry.md`, `bee872aa`). cc-0019 CORRECTED: Unit A IMPLEMENTED (live `m.is_publish_eligible` + `fill_pending_slots` gate, migration `20260524091020`, D-01 `a2e5f3f5` + PK approval per header) — Unit B (ai-worker preflight) verified NOT implemented in deployed ai-worker v2.12.0; v3.05's "brief only" was stale. IG-arc OWED 4-way close discharged (publisher + migration legs). Session `1219aff0`. sync_state + action_list patched read-HEAD-first with blob SHAs passed. Dashboard PHASES leg N/A (operational). Next: YT F-YT-FAILED-NO-RETRY plan_review D-01.**
