...
|---|---|---|---|---|
| 1 | **cc-0015 friction-pool-view — IN EXECUTION (v3.12)** | **P2, rank 1** | Gate PK-confirmed (cc-0014 early-close D-IOL-001). **Stage A APPLIED** (mig `cc_0015_a_pool_schema_additions` v`20260526110657`; D-01 `58d89efe`; V-A1–A4 PASS). **Stage F SHIPPED — CCB V-F1/V-F2 PASS, merged + deployed to production** (dashboard main `f9d51a96`; Vercel prod `dpl_FftxSjeBNyurxZetJedi3zDsan6b` READY @ `dashboard.invegent.com`; category per-option DB descriptions RE-SCOPED follow-up; FAB capture_reason absence accepted). **Stage B RPC APPLIED** (COEXIST; `fn_recent_cases` filter extension, mig `cc_0015_b_fn_recent_cases_filters` v`20260526124005`; plan_review `58d89efe` + sql_destructive `17db8b27` + PK phrase; V-B2 byte-identical; file backfilled). **Stage B FRONTEND = next separate gate** (filter bar + saved views + category-desc hydration on `/operations`). **Stage C/D/E excluded; `/operations/pools` v0 untouched.** **cc-0015 NOT complete.** | PK → CCD | (a) ✅ Stage F live + CCB smoke PASS; ✅ COEXIST decided + Stage B RPC applied/verified; (b) **next: build Stage B frontend** → its own plan/ef_deploy + merge, each PK-gated; (c) Stage A.5 backfill recat + Stage C/D/E when PK directs. |
...
| Rank | Item | Priority | Why now | Next action |
|---|---|---|---|---|
| D1 | **cc-0015 friction-pool-view UI** | P2, IN EXECUTION v3.12 | Stage A schema APPLIED (verified). **Stage F (operator copy) SHIPPED** — CCB V-F1/V-F2 PASS → FF-merged to dashboard main `f9d51a96` → production deploy READY (`dashboard.invegent.com`); category per-option DB descriptions RE-SCOPED follow-up. **Stage B/C/D (the pool-view UI) HELD for re-brief** — collides with the shipped `/operations/pools` Repair-Board v0 (notes-`pool_key` model vs cc-0015 category model); v0 NOT superseded. | PK → CCD | ✅ CCB prod smoke PASS. Stage B reconciliation re-brief AUTHORED (`cc-0015b`) → PK decides supersede/coexist/re-scope; category-description follow-up. |
