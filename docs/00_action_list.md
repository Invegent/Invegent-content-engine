...
|---|---|---|---|---|
| 1 | **cc-0015 friction-pool-view — IN EXECUTION (v3.12)** | **P2, rank 1** | Gate PK-confirmed (cc-0014 early-close D-IOL-001). **Stage A APPLIED** (mig `cc_0015_a_pool_schema_additions` v`20260526110657`; D-01 `58d89efe`; V-A1–A4 PASS). **Stage F SHIPPED — CCB V-F1/V-F2 PASS, merged + deployed to production** (dashboard main `f9d51a96`; Vercel prod `dpl_FftxSjeBNyurxZetJedi3zDsan6b` READY @ `dashboard.invegent.com`; category per-option DB descriptions RE-SCOPED follow-up; FAB capture_reason absence accepted). **Stage B/C/D HELD for re-brief** (category pools vs Repair-Board v0; v0 NOT superseded). **cc-0015 NOT complete** (Stage A schema + Stage F copy done; B/C/D + category follow-up + Stage A.5 backfill + D/E/G open). | PK → CCD | (a) ✅ **CCB prod smoke PASS** (Stage F live+clean); (b) **Stage B reconciliation re-brief AUTHORED** `docs/briefs/cc-0015b-pool-view-reconciliation.md` → PK decides supersede / coexist / re-scope; (c) Stage A.5 backfill recat + category-description follow-up when PK confirms. |
