...
| Rank | Item | Priority | Why now | Next action |
|---|---|---|---|---|
| D1 | **cc-0015 friction-pool-view UI** | P2, IN EXECUTION v3.13.2 | Stage A schema APPLIED (verified). **Stage A.5 recategorisation APPLIED + verified (v3.13.1)** — 3 dashboard-UI cases moved `operator_friction`→`dashboard_ui` (mig `20260526222125`; D-01 `bbe9a0fa`; `41576f8`); now visible under the live `/operations?category=dashboard_ui` saved view. **Stage F (operator copy) SHIPPED** — CCB V-F1/V-F2 PASS → dashboard main `f9d51a96` → prod READY. **Stage B RPC APPLIED + Stage B FRONTEND MERGED + DEPLOYED (v3.13)** — COEXIST model executed: `/operations` filter bar + 5 saved views + URL-param filters + category-desc hydration; CCB preview V-B3/V-B4/V-B5/V-B6 PASS → FF-merge dashboard main `25bcdb7` → production deploy READY (`dpl_Hb2BkFnK3cL4cCAHXbkAz1tDnHjW`, `dashboard.invegent.com`). **Stage G process doc AUTHORED (v3.13.2)** — `docs/process/ICE-PROC-002-pooled-resolution.md` (doc-only; V-G1 PK approval remains). **Repair-Board v0 (`/operations/pools`) untouched (V-B5) — NOT superseded.** Stage C/D/E held. | PK → CCD | ✅ Stage A.5 + Stage B frontend live + Stage G doc authored. Next: Stage C/D/E (each PK-gated); source/sort + FAB hydration deferred; ICE-PROC-002 V-G1 + +4wk review. |
