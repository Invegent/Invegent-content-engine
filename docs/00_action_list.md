...
**v3.15 cycle: ~docs-only session close** (recorded two parked lanes — Instagram diagnosis [CCH lane] + cc-0020 subscription-email-ingest [CCD lane]). **0 production mutation. 0 SQL. 0 migration apply. 0 DB connection. 0 deploy. 0 Gmail/OAuth/token. 0 cron/Pub-Sub. 0 PR/merge. 0 D-01.** Docs-only on `main`: v3.15 session note + `00_` sync_state/action_list. cc-0020 draft migration UNAPPLIED; branch `feat/cc-0020-subscription-email-ingest` @ `7497e74` pushed/unmerged/undeployed.
...
**Carries flagged (NOT actively ranked):** **Q-nightly-health-check-v1-005 (OPEN, non-blocking, next-fire watch; brief v3.1.1)**; **OWED IG-publish-restore arc 4-way close (pre-compaction 2026-05-24)** *(v3.06: substantially discharged)*; 3 no-fire scheduler days (2026-05-16/18/19, P3 informational); **doc-patch debt residual (v3.07): cc-0017a v1.2 / cc-0010A / cc-0011 / cc-0012 / cc-0017c-vchecks.md V-B4 → CC surgical pass; lessons promotion → next-lesson-cycle/CC; decisions.md → locate first.** **v3.15 parked lanes (NOT actively ranked):** **cc-0020 subscription-email-ingest** (CCD lane; branch `feat/cc-0020-subscription-email-ingest` @ `7497e74` pushed/unmerged/undeployed; Stage 3a DB-layer validation OPEN → Stage 4 HELD; draft migration UNAPPLIED; awaits PK/CCH release; **UI destination (PK 2026-05-27): the review + spend-history UI extends the live `invegent-dashboard` area `dashboard.invegent.com/system/subscriptions` on its own branch when Stage 4 releases — not a local surface, not the PRV pattern; backend/ingest stays in content-engine; Stage 4 read path into `k.*` = `public` SECURITY DEFINER RPC or service-role route, no direct client/PostgREST**); **Instagram "not publishing" diagnosis** (CCH lane; runbook `b911d91`; NY/PP `publish_enabled` flipped via UI → cc-0019 fill-gate cleared; self-heal ≈ 2026-05-29; closed unless PK reopens).
