...
| F-YT-FAILED-NO-RETRY | publisher catch sets transient auth failure to terminal `failed` (never re-selected) → freezes backlog silently. Brief authored + plan_review CLEAN v3.06 (`bee872aa`→`a3f329f3`; `abf6f63c`→`f89c23cf`). **FULL UNIT B COMPLETE v3.10.** v1.8.0 deploy (`818ee949`) + B1 5-quota (`33eeb164`, mig `…b1_only`) v3.09; v1.9.0 deploy (`4719f53b`, predicate → `IN ('approved','published')`) + B2 17 token-casualty (`2135ae11` + PK phrase, mig `…b2_token_casualty_20260526`; `failed` 31→9 = B3-only) v3.10. B3 9 = onboarding debt. 🔴 NEW finding **F-YT-PUB-PUBLISH-AUDIT-GAP** (cross-posted YT publishes get no `m.post_publish` row — see STATUS BLOCK). |
| 🔴 F-YT-PUB-PUBLISH-AUDIT-GAP (NEW v3.10) | Cross-posted drafts (already FB-published) that publish to YouTube are marked `video_status='published'` with a real `youtube_video_id` but get **NO `m.post_publish` youtube row**. Root cause: `m.post_publish` `uq_publish_attempt UNIQUE(post_draft_id, attempt_no)` + the publisher hardcodes `attempt_no=1` → collides with the existing FB row → INSERT rejected; `insertErr` logged-not-thrown (index.ts:258). Uploads succeed (videos live) + no double-upload (SELECT gated on `video_status='generated'`), but the YT audit trail is incomplete. Surfaced by v1.9.0 routing `published` drafts to YouTube; all 17 B2 affected as they drain. Backfillable from `draft_format.youtube_video_id`. **Cohort = 11** (2 B2-recovered + **9 pre-existing historical** cross-posted publishes; all 11 have a FB row at attempt_no=1). Fix validated by the lone draft with both FB+YT rows — its YT row uses `attempt_no=2`. **Brief AUTHORED (candidate, gated):** `docs/briefs/yt-publisher-publish-audit-gap.md` — Unit A v1.10.0 (next-available `attempt_no` + surface insertErr; ef_deploy) → Unit B sql_destructive backfill, A-then-B. First 2 affected: YouTube `VonxZesS0Ws`, `u-xYWGg1qWI`. |
