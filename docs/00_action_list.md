...
**v3.13.3 cycle: ~doc record** (cc-0015 Stage G ICE-PROC-002 V-G1 PK approval recorded in the doc + `00_`/brief). **0 production mutation. 0 SQL. 0 D-01.** `00_`-only close. Stage G now complete as a doc artefact (V-G1/V-G2/V-G3 satisfied); next milestone = +4-week review ≈ 2026-06-23.
...
- **ICE-PROC-002 +4-week process review (v3.13.3)** — due ≈ **2026-06-23** (fourth Friday pool session). Evaluate: are the 5 saved views useful; should Stage C/D/E proceed; should source filter / sort / FAB category-desc hydration be promoted. Process check only — not a cc-0015 completion gate. Soft calendar item.
