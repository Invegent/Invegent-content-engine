...
| F-YT-FAILED-NO-RETRY | publisher v1.6.0 catch sets transient auth failure to terminal `failed` (never re-selected) → freezes backlog silently. Brief authored + plan_review CLEAN v3.06 (`bee872aa`→`a3f329f3`; `abf6f63c`→`f89c23cf`); execution-gated. |
...
| **F-YT-PUB-AVATAR-EXCLUSION** | youtube-publisher format filter → avatar enabled | P2 carry | **✅ SHIPPED/CLOSED 2026-05-25** — youtube-publisher **v1.7.0** (`21e372d`, EF v47) allow-list now includes `video_short_avatar`; portrait draft **40f9fa25** published **sfQvSM2Osus** (unlisted, NDIS-Yarns); landscape **ba5b34eb** retired (`archived_stale`). Full chain ai-worker v2.13.0 → heygen-worker v2.0.0 (async) → youtube-publisher v1.7.0 OPERATIONAL. D-01 `c76aea38` (GNB/L46, closed). See `docs/operations/avatar-youtube-pipeline-status-2026-05-25.md`. | n/a (closed) | — |
| **Avatar→YouTube pipeline — future enhancements** | portrait-native avatars (full-bleed, remove letterbox bands); unlisted→public privacy decision; render monitoring/tuning (`STALE_RENDER_MAX_MS`, cron cadence, MAX_SUBMITS/POLLS) | **P3 future enhancement** | OPEN (non-blocking) — pipeline operational; these are polish/ops items, NOT blockers, NOT "avatar broken". | PK → chat | See `docs/operations/avatar-youtube-pipeline-status-2026-05-25.md` §4. |
