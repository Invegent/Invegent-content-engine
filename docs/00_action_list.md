...
- **F-YT-FAILED-NO-RETRY** — **Unit A DEPLOYED + verified v3.09** (`youtube-publisher` **v1.8.0** live, `--no-verify-jwt`; from `main` `b983a38` = EF commit `631fa93a`; D-01 `ef_deploy` `818ee949` + PK directive; live GET `youtube-publisher-v1.8.0` HTTP 200). plan_review CLEAN v3.06 (`abf6f63c`→`f89c23cf`; brief `bee872aa`→`a3f329f3`). **Unit B1-only APPLIED + CCD-verified v3.09** — migration `yt_failed_backlog_recovery_unit_b_b1_only_20260526` (v`20260526054427`), D-01 `sql_destructive` `33eeb164…` (`agree`/`completed`); **5 quota drafts recovered** (`failed` 31→26; 0 quota remain). **B2 17 token-casualty + B3 9 never-connected UNTOUCHED.** Next: **Unit B-2** (B2 17 reset + `published→approved`) when PK directs — fresh sql_destructive + PK approval. Full Unit B NOT complete; B3 = onboarding debt.
...
| F-YT-FAILED-NO-RETRY | publisher catch sets transient auth failure to terminal `failed` (never re-selected) → freezes backlog silently. Brief authored + plan_review CLEAN v3.06 (`bee872aa`→`a3f329f3`; `abf6f63c`→`f89c23cf`). **Unit A DEPLOYED + verified v3.09** (`youtube-publisher` v1.8.0 live, `--no-verify-jwt`; D-01 `ef_deploy` `818ee949`). **Unit B1-only APPLIED + CCD-verified v3.09** (mig `yt_failed_backlog_recovery_unit_b_b1_only_20260526`; D-01 `sql_destructive` `33eeb164…`; 5 quota recovered, `failed` 31→26). **B2 17 + B3 9 UNTOUCHED.** A-then-B: A done, B1 done; B2 held for Unit B-2; full Unit B NOT complete. |
...
| Open follow-on | F-YT-FAILED-NO-RETRY → youtube-publisher **v1.8.0 DEPLOYED + verified live v3.09** (Unit A done; the publisher table above shows the v3.08-milestone version). **Unit B1-only APPLIED + CCD-verified** (5 quota recovered; mig `…unit_b_b1_only_20260526`; D-01 `33eeb164…`). **B2 17 held for Unit B-2; B3 9 onboarding debt.** Full Unit B NOT complete. |
