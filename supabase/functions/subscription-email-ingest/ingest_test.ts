/**
 * cc-0020 Subscription Email Ingest — ingest scaffold tests (Stage 3).
 *
...
Deno.test("manual ingest: 4 created, 3 low-confidence skipped", () => {
  const repo = new InMemoryIngestRepository();
  const counts = ingestEmails(ALL_FIXTURES, repo);
  ...
  assertEquals(repo.listCandidates().length, 4, "stored candidates");
});

// 2/3 — Gmail fetch is disabled/fail-closed by default; no live access
Deno.test("gmail fetch: fail-closed (disabled) by default", () => {
  const g = fetchSubscriptionEmails();
...
Deno.test("idempotent ingest: same gmail_message_id is a no-op", () => {
  const repo = new InMemoryIngestRepository();
  const first = ingestEmails([monthlyReceipt], repo);
  assertEquals(first.candidates_created, 1, "first inserts one");
  const second = ingestEmails([monthlyReceiptDuplicate], repo); // same message re-fetched
  ...
  assertEquals(repo.listCandidates().length, 1, "still one candidate");
});

// 6 — re-promoting the same candidate creates only one spend event
Deno.test("idempotent promotion: one spend_event per candidate", () => {
  const repo = new InMemoryIngestRepository();
  ingestEmails([monthlyReceipt], repo);
  const cand = repo.listCandidates()[0];

  const p1 = promoteCandidate(cand.candidate_id, repo);
  assertEquals(p1.created, true, "first promotion creates");
  const p2 = promoteCandidate(cand.candidate_id, repo);
  ...
  assertEquals(repo.listSpendEvents().length, 1, "exactly one spend event");
  assertEquals(repo.getCandidate(cand.candidate_id)?.review_status, "accepted", "candidate accepted");

  const se = repo.listSpendEvents()[0];
...
Deno.test("promotion refuses candidate with no amount (amount_original NOT NULL)", () => {
  const repo = new InMemoryIngestRepository();
  const row = toCandidateRow(missingAmount, parseEmail(missingAmount));
  assertEquals(row.amount, null, "missing amount maps to null");
  const ins = repo.insertCandidateOnConflictDoNothing(row); // inserted directly (bypassing confidence gate)

  let threw = false;
  try {
    promoteCandidate(ins.candidate_id, repo);
  } catch (e) {
    threw = e instanceof CandidateNotPromotableError;
  }
  assert(threw, "throws CandidateNotPromotableError");
  assertEquals(repo.listSpendEvents().length, 0, "no spend event created");
});
