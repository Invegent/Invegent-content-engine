// ingest — RSS / YouTube channel feed ingestion EF
// Sourced from deployed version 110 ("ingest-v8-youtube-channel") on 22 Apr 2026
// as part of Q2 closure. Brings the function under version control.
// Do not modify in this PR — Q2 only sources it. Functional changes go in separate PRs.
...
const VERSION = "ingest-v8-youtube-channel";
...
/* ─── YouTube Channel Source Handler ─── */
...
async function runYouTubeChannelSource(supabase: any, source_id: string, flags: RunFlags) {
  const pubClient = getSupabasePublicClient();

  const { data: src, error: srcErr } = await supabase
    .from("feed_source")
    .select("source_id, source_name, source_type_code, config")
    .eq("source_id", source_id)
    .maybeSingle();
  ...
  if (src.source_type_code !== "youtube_channel") {
    return { ok: false, source_id, step: "validate_source_type", error: `Expected youtube_channel, got ${src.source_type_code}` };
  }

  const channel_id = src.config?.channel_id as string | undefined;
  if (!channel_id) {
    return { ok: false, source_id, step: "validate_config", error: "config.channel_id is missing" };
  }

  const max_videos = Number(src.config?.max_videos_per_run ?? 3);
  const rssUrl = `https://www.youtube.com/feeds/videos.xml?channel_id=${channel_id}`;

  // Fetch and parse the channel RSS feed
  let items: any[] = [];
  try {
    ...
    if (!resp.ok) {
      return { ok: false, source_id, step: "fetch_rss", error: `Fetch failed: ${resp.status} ${resp.statusText}` };
    }

    const xml = await resp.text();
    items = parseRss(xml, max_videos * 3); // fetch more than needed so we can skip already-analysed
  } catch (e: any) {
    return { ok: false, source_id, step: "fetch_or_parse", error: e?.message ?? String(e) };
  }

  let videos_checked = 0;
  let videos_new = 0;
  let videos_analysed = 0;
  let videos_failed = 0;
  const errors: string[] = [];

  for (const item of items) {
    if (videos_analysed >= max_videos) break;
    ...
    // Check if already analysed via exec_sql (f schema not PostgREST-exposed)
    try {
      const { data } = await pubClient.rpc("exec_sql", {
      ...
      // If check fails, try to analyse anyway
    }

    videos_new += 1;

    // Call video-analyser Edge Function
    try {
      const { data, error } = await pubClient.functions.invoke("video-analyser", {
        body: { youtube_url: link, client_id: null },
      });
      ...
      videos_failed += 1;
      errors.push(`${videoId}: ${e?.message ?? String(e)}`);
    }
  }
  ...
  return {
    ...
    source_id,
    channel_id,
    channel_name: src.config?.channel_name ?? src.source_name,
    videos_checked,
    videos_new,
    videos_analysed,
    videos_failed,
...
app.post("/ingest/ingest/run", async (c) => {
  ...
  // Check source type to route appropriately
  const { data: srcCheck } = await supabase
    .from("feed_source")
    .select("source_type_code")
    .eq("source_id", source_id)
    .maybeSingle();

  if (srcCheck?.source_type_code === "youtube_channel") {
    const result = await runYouTubeChannelSource(supabase, source_id, {
      max_items,
      write,
      normalize,
      canonicalize,
...
app.post("/ingest/ingest/run-all", async (c) => {
  ...
  // Fetch RSS content_item sources (existing behaviour)
  const { data: rssSources, error: rssErr } = await supabase
    .from("feed_source")
    .select("source_id, source_name, output_kind, status, source_type_code")
    .eq("output_kind", "content_item")
    .eq("status", "active");

  if (rssErr) return c.json({ ok: false, step: "load_sources", error: rssErr }, 500, corsHeaders);

  // Fetch youtube_channel sources
  const { data: ytSources, error: ytErr } = await supabase
    .from("feed_source")
    .select("source_id, source_name, output_kind, status, source_type_code")
    .eq("source_type_code", "youtube_channel")
    .eq("status", "active");

  if (ytErr) return c.json({ ok: false, step: "load_yt_sources", error: ytErr }, 500, corsHeaders);

  const rssList = (rssSources ?? []).slice(0, limit_sources > 0 ? limit_sources : undefined);
  const ytList = ytSources ?? [];

  const results: any[] = [];

  // Process RSS sources
  for (const s of rssList) {
    try {
  ...
  // Process YouTube channel sources
  for (const s of ytList) {
    try {
      const r = await runYouTubeChannelSource(supabase, s.source_id, {
        max_items,
        write,
        normalize,
        canonicalize,
  ...
  return c.json(
    {
      ...
      rss_sources: rssList.length,
      youtube_sources: ytList.length,
      results,
      path_seen: c.req.path,
