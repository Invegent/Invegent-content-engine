// creative_contract_test.ts — ACI Foundation v0 / Slice B1 (ai-worker; vendored per-worker copy).
//
...
//
// B1 NOTE: ai-worker has no b1_production.ts of its own. Test 6 (no-drift consistency) imports
// the B1_* runtime constants from ../image-worker/b1_production.ts as a TEST-ONLY cross-dir
// import. Tests are NOT deployed, so this does NOT violate the no-_shared / no cross-function
// runtime-import rule. All 9 test intents are preserved unchanged.
...
} from '../image-worker/b1_production.ts'; // TEST-ONLY cross-dir import (ai-worker has no b1_production.ts; tests are not deployed)
...
// Test 10 — TMR D7 N7b: NDIS_IMAGE_QUOTE_NEWS_CARD_V1 is registered and resolves for the
