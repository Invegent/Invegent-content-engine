-- Migration: create_resolve_slot_assets_v1
-- Creative Asset Selection Slice-1 — public.resolve_slot_assets (read-only slot-resolver RPC)
--
...
--   · p_format is accepted but UNUSED in v1 — format→template mapping belongs to Template
...
--     recorded here, deferred to intake; Slice-1 uses the single existing governed logo.
...
--   · Aspect filtering is intentionally LOOSE in v1 (Creatomate crops) — no aspect rejection.
--   · Scrim opacity constants (needs_scrim → 64, true → 40) are PROVISIONAL / to_be_calibrated.
...
--
-- ⛔ APPLY IS PK-GATED. Applied 2026-07-03 on explicit PK approval pinned to
--    sha256 b6a0589ee340b303e6b616a8527b0d8ca6957505405dc41d50b39bcbef98c7ae.
--    Ships dark: no production consumer, no worker/runtime change.
--
...
AS $$
DECLARE
  -- Scrim opacity constants: PROVISIONAL / to_be_calibrated (v0 result §7, gate-1 decision 2).
  c_scrim_opacity_needs_scrim CONSTANT numeric := 64;  -- selected background safe_for_text_overlay = 'needs_scrim'
  c_scrim_opacity_text_safe   CONSTANT numeric := 40;  -- selected background safe_for_text_overlay = 'true'
  ...
  IF v_has_background THEN
    ...
    IF v_has_scrim THEN
      v_mods := v_mods || jsonb_build_object(
        'Scrim.opacity',
        CASE WHEN v_pick->>'sfto' = 'needs_scrim'
...
'Creative Asset Selection Slice-1: read-only governed slot resolver (background/logo/scrim) for a TMR registry template. Fail-closed with per-asset reason codes; p_format accepted but unused in v1; scrim constants to_be_calibrated. Service-role-only. Ships dark (no production consumer).';
