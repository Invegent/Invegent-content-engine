-- =====================================================================
-- LEDGER BACKFILL (not a new apply) — public.get_client_production_readiness_queue
-- governed-exempt (D1) rider
--
-- CONTEXT: B2 visual-verdict-promotion Stage 0 forensic reconstruction
-- (2026-08-01). The originating "B2" lane that authored this rider applied
-- it live and was lost with its deleted worktree before a migration file
-- was committed. This file backfills the ledger to match verified live
-- truth — it does NOT change production behaviour (the rider has been live
-- since before this backfill was authored) and is NOT itself an apply
-- authorisation for anything downstream.
--
-- EVIDENCE (live DB, 2026-08-01, this session):
--   * pg_get_functiondef('public.get_client_production_readiness_queue(text)')
--     diffed line-for-line against the tracked predecessor
--     (20260730120000_client_production_readiness_queue_rpc_v1.sql). The
--     ONLY delta is the block backfilled below — no other function, table,
--     grant, or column differs between live and tracked.
--   * public.is_capability_exempt_format(text) — the predicate this rider
--     calls — is NOT new: it was created by the already-tracked
--     20260729143000_s9_layer1_capability_gate_fill_pending_slots.sql and is
--     byte-identical live vs. tracked. Only the readiness-queue RPC itself
--     was undocumented drift.
--   * Matches D1's decided execution exactly (Creatomate Global Ultimate
--     programme brief §1.2: "declare the template-less carve-out
--     (render_engine='none', capability-exempt, exemption set exactly
--     {text}) ... wherever platform_support marks text supported"). Live
--     verification: all 8 committed D1 cells (4 brands x FB/LI text) return
--     capability_status_overlay='governed_exempt', overall_state='ready'.
--
-- WHAT THIS RIDER ADDS (vs. the tracked 20260730120000 body):
--   1. A new `exempt_cells` CTE — surfaces (platform, format) pairs that are
--      capability-exempt (render_engine='none') and platform_support=true,
--      even when no client_format_config/schedule/slot demand exists yet,
--      so a governed template-less format still appears in the queue.
--   2. `is_governed_exempt` computed column on cells_computed (calls
--      public.is_capability_exempt_format(ac.format) AND the same
--      platform_support check used in exempt_cells).
--   3. Four downstream CASE branches (responsible_lane, next_required_outcome,
--      missing_proof_or_gate, overall_state) that special-case
--      is_governed_exempt cells whose capability_status would otherwise read
--      template_missing/unsupported_silent_degrade: they now resolve to
--      "ready"/"None — governed template-less path" when a publisher
--      profile+schedule row exists, or route to publisher_onboarding when it
--      doesn't — instead of being misreported as a template gap.
--   4. One new output field, `capability_status_overlay` ('governed_exempt'
--      or NULL), rendered ALONGSIDE capability_status, never replacing it
--      (same "always alongside, never in place of" convention the original
--      migration used for overall_state).
--
-- NOT reproduced/changed by this backfill: classify_format_capability,
-- select_template, resolve_slot_assets, grants, security posture (still
-- SECURITY DEFINER / STABLE / search_path='' / service_role-only — unchanged
-- from the tracked predecessor), or any other function/table.
--
-- (R) ROLLBACK (reference only — see the paired
--   ROLLBACK_20260801120000_backfill_readiness_queue_governed_exempt_rider_v1.sql):
--   restores the exact tracked-predecessor function body (pre-rider), i.e.
--   the 20260730120000 CREATE OR REPLACE, byte-identical.
-- =====================================================================
...
BEGIN
  ...
  -- ---- D1 RIDER: governed template-less (capability-exempt) cells surface even
  -- ---- with zero config/schedule/slot demand — see header. ----
  exempt_cells AS (
    SELECT p.platform, cf.ice_format_key AS format, false AS is_probe_cell
    FROM platforms AS p
    CROSS JOIN t."5.3_content_format" AS cf
    WHERE cf.is_active = true
      AND public.is_capability_exempt_format(cf.ice_format_key)
      AND COALESCE((cf.platform_support ->> p.platform)::boolean, false)
  ),
  all_cells AS (
    ...
    UNION
    SELECT platform, format, is_probe_cell FROM exempt_cells
  ),
  profile_rows AS (
  ...
  cells_computed AS (
    SELECT
      ...
      -- ---- D1 RIDER: governed template-less exemption — see header. ----
      (
        public.is_capability_exempt_format(ac.format)
        AND EXISTS (
          SELECT 1 FROM t."5.3_content_format" AS cf
           WHERE cf.ice_format_key = ac.format
             AND cf.is_active = true
  ...
  routed_rows AS (
    SELECT
      ...
      CASE
        ...
        -- ---- D1 RIDER: governed-exempt overrides below, before the generic template_missing branch. ----
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade')
             AND NOT (fr.has_profile_row AND fr.has_schedule_row) THEN 'publisher_onboarding'
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade') THEN NULL
      ...
      CASE
        ...
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade')
             AND fr.has_profile_row AND fr.has_schedule_row THEN 'None — governed template-less path, cell is production ready'
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade') THEN 'Create publisher profile + schedule row (publisher onboarding) — format is governed template-less, no template needed'
      ...
      CASE
        ...
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade')
             AND fr.has_profile_row AND fr.has_schedule_row THEN NULL
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade') THEN 'No c.client_publish_profile/schedule row exists for this platform (format is governed template-less; no template gap)'
      ...
      CASE
        ...
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade')
             AND fr.has_profile_row AND fr.has_schedule_row THEN 'ready'
        WHEN fr.is_governed_exempt AND fr.capability_status IN ('template_missing','unsupported_silent_degrade') THEN 'not_configured'
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'capability_status_overlay',    CASE WHEN r.is_governed_exempt AND r.capability_status IN ('template_missing','unsupported_silent_degrade') THEN 'governed_exempt' ELSE NULL END,
...
COMMENT ON FUNCTION public.get_client_production_readiness_queue(text) IS
  ...
  'platform_support x client_format_config (runtime_reachable), plus the D1 governed-exempt '
  '(is_capability_exempt_format) overlay: capability_status_overlay=''governed_exempt'' and '
  'overall_state=''ready'' for template-less-governed cells even when the underlying classifier '
  'would otherwise report template_missing/unsupported_silent_degrade. '
  ...
  'untouched return value except for the governed-exempt overlay above. Service-role only. Ships '
  'dark (no dashboard consumer wired). Brief: cc-0088-client-production-readiness-queue-brief-v1.md '
  '+ Creatomate Global Ultimate programme brief v1 (D1). Ledger-backfilled by '
  '20260801120000_backfill_readiness_queue_governed_exempt_rider_v1.sql — rider was already live '
  '(undocumented drift) at backfill time; see file header for verification method.';

-- Grants unchanged from the tracked predecessor (service_role-only) — re-asserted defensively,
...
-- file ROLLBACK_20260801120000_backfill_readiness_queue_governed_exempt_rider_v1.sql):
-- restores the exact tracked-predecessor (pre-rider) function body.
-- =====================================================================
