...
BEGIN
  ...
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'Rollback (CFW): expected exactly 1 proof_event row deleted, got %.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
END $$;
...
BEGIN
  ...
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'Rollback (CFW): expected exactly 1 row reverted (state guard: visually_approved AND approved_by=PK), got %. Row state has drifted since forward-apply — STOP, do not force.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
END $$;
