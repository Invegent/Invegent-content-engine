...
-- (per the standing rule in CLAUDE.md: "supabase/migrations/ is not reliable ground truth for live
-- DB state" — see memory note migration-ledger-git-drift). Writing this file performs NO database
-- mutation.
--
-- WHAT THIS RECORDS: "B-roll PARITY Activation v1" — the live repoint of PP governed
-- `video_short_stat` to the governed B-roll template (`dd5fd75e` / provider `46c5c4ac`), at the
-- incumbent's full 1080x1920/12s output contract (closed by video-worker v3.15.0's render-time
-- output-parity overlay). 3 DML rows on c.creative_template_variant_candidate. NO DDL.
--
-- VERSION CHOSEN (20260729053002): the live `updated_at`/`reviewed_at` timestamp stamped on all 3
-- affected rows by the apply itself — `2026-07-29 05:30:02.033359+00`, confirmed by db-rls-auditor
-- read against live state during this reconciliation lane (2026-07-29). This apply was executed via
-- a direct `execute_sql` call, NOT `apply_migration` — no `supabase_migrations.schema_migrations`
-- ledger row exists for it (unlike the six timestamp-drifted migrations reconciled alongside this
-- one, which DO have ledger rows). This file's version is therefore a reconciliation-chosen
-- identity, not a re-stated ledger value.
--
-- SOURCE (byte-exact copy, zero changes below the "BEGIN;" line):
--   docs/briefs/artifacts/broll-parity-activation-v1-forward.sql
--   (git-tracked since commit d5ddca1 / packet docs/briefs/broll-parity-activation-v1-apply-packet.md)
--
-- GROUNDING EVIDENCE (all re-verified live 2026-07-29 by db-rls-auditor during this reconciliation):
--   * c.creative_template_variant_candidate, format_key='video_short_stat', exactly 3 rows:
--       8b611275-ea75-4663-a411-f05a329dfd5d / dd5fd75e-982d-4c3d-89cd-7ce0936076b2 = strong_candidate
--       b61e2f15-0213-4db8-b401-8629ae11313a / a3d8472d-9438-4312-9f11-b6a920be4014 = candidate
--       dee47d2e-7a65-424f-a389-3165f8c22bd2 / 4cd2c9e2-bb55-4a71-9f13-cb2e1c41e958 = candidate
--     — exact match to this file's post-state, all three timestamped 2026-07-29 05:30:02.033359+00.
--   * Production result record: docs/briefs/results/broll-parity-activation-v1-result.md (register v6.54).
--   * Production register: docs/00_sync_state.md v6.54 entry; memory broll-production-activation-live.md.
--
-- ⚠ REPLAY WARNING: this file is SELF-GUARDING, not inert. Its own step (0) pre-image check will
-- RAISE EXCEPTION and abort cleanly if run against the CURRENT live state, because the pre-image it
-- expects (the pre-activation fit_status set) no longer exists — the activation already happened.
-- This is the CORRECT and EXPECTED behavior for a reconciliation record: it cannot be silently
-- re-applied. Do not "fix" the guard to make this file re-runnable; do not use this file as a
-- template for a NEW change (author a new migration with a new version+name instead, per the
-- standing "migration name = permanent identity" rule).
-- ================================================================================================

-- ============================================================================================
-- B-roll PARITY Activation v1 — repoint PP governed video_short_stat to the B-roll template,
-- now that the render-time output-parity overlay makes it render at the INCUMBENT'S output spec.
-- Tier T3 · DML ONLY on c.creative_template_variant_candidate (3 rows) · NO DDL · NO EF deploy here.
--
-- Lineage: this is the v6.48 activation (`_harness/cc_broll_activation_20260728/forward.sql`, REV 3)
-- re-cut. That apply worked and was rolled back ONLY because the promoted template rendered
-- 720x1280/8s against the incumbent's 1080x1920/12s (TPR-1). video-worker v3.15.0 closes that gap in
-- code. The row-level mechanics, the pre-image digest and every guard below are UNCHANGED and still
-- live-valid: the live pre-image digest recomputed 2026-07-29 is 962043fb55bdd2a1a9e5d0a8718118e0,
-- byte-identical to the value frozen before the v6.48 apply — proving that rollback was digest-exact
-- and that this file is operating on exactly the state it was written for.
--
-- ⛔ ORDERING IS A HARD PRECONDITION, NOT A PREFERENCE ⛔
--    video-worker v3.15.0 MUST be deployed and verified BEFORE this file is applied. SQL cannot see
--    edge-function code, so this file CANNOT enforce that — it is STOP-1 in the apply packet and is
--    owned by the operator. Applying this DML against v3.14.0 reproduces the exact v6.48 regression:
--    production would immediately render 720x1280/8s.
--
...
--   (3d) NEW vs v6.48 — the demoted incumbent must still rank as the FIRST fallback, so a
--        fail-closed B-roll slot degrades to the 1080x1920/12s incumbent instead of dying else ABORT
...
DECLARE v_win uuid;
BEGIN
  SELECT (public.select_template('property-pulse', NULL, 'video_short_stat', NULL, 'parity-activation-baseline')
...
BEGIN
  ...
  UPDATE c.creative_template_variant_candidate
     SET fit_status = 'strong_candidate',
         fit_reason = 'B-roll PARITY Activation v1 (PK 2026-07-29): promoted to PP default for video_short_stat. Requires video-worker >= v3.15.0, whose render-time output-parity overlay makes provider template 46c5c4ac render 1080x1920/12s (TPR-1 parity with the incumbent). Supersedes the v6.48 activation, rolled back for the 720p/8s output-spec regression.',
         reviewed_by = 'PK',
         reviewed_at = now(),
         updated_at  = now()
  ...
  -- 2b. DEMOTE incumbent 1 (this is what actually lets B-roll win — promoting alone cannot, because
  -- the B-roll template is the NEWEST and ranking inside a bucket is template created_at ASC).
  UPDATE c.creative_template_variant_candidate
     SET fit_status = 'candidate',
         fit_reason = 'B-roll PARITY Activation v1 (PK 2026-07-29): demoted from strong_candidate so the B-roll variant ranks first. Remains fully selectable as the first fallback (1080x1920/12s).',
         reviewed_by = 'PK',
         reviewed_at = now(),
         updated_at  = now()
  ...
  UPDATE c.creative_template_variant_candidate
     SET fit_status = 'candidate',
         fit_reason = 'B-roll PARITY Activation v1 (PK 2026-07-29): demoted from strong_candidate so the B-roll variant ranks first. Remains fully selectable as a fallback.',
         reviewed_by = 'PK',
         reviewed_at = now(),
         updated_at  = now()
...
-- Asserts the OUTCOME, not the intent: B-roll must actually win the production call signature, must
-- actually resolve a VIDEO clip (guards the resolver-v1.3-class "video field filled with a still"),
-- and the incumbent must remain the first fallback.
DO $postverify$
DECLARE
  v_sel    jsonb;
  v_win    uuid;
...
BEGIN
  ...
  v_sel := public.select_template('property-pulse', NULL, 'video_short_stat', NULL, 'parity-activation-postverify');
  ...
  -- (3d) NEW: the demoted incumbent must remain the FIRST fallback. A demoted row is still fully
  -- selectable, so a B-roll slot that ever fails closed degrades to the 1080x1920/12s incumbent
  -- rather than dying. This is the in-transaction evidence for "the incumbent stays rollback-ready".
  v_alt1 := (v_sel -> 'alternatives' -> 0 ->> 'template_id')::uuid;
  IF v_alt1 IS DISTINCT FROM 'a3d8472d-9438-4312-9f11-b6a920be4014'::uuid THEN
    RAISE EXCEPTION 'ABORT (3d): first fallback is %, expected incumbent a3d8472d. The degradation path is not the parity-spec incumbent — STOP.', v_alt1;
  END IF;

  RAISE NOTICE '(3) post-verify OK: winner=dd5fd75e background=% first_fallback=a3d8472d', v_bg;
END
$postverify$;
