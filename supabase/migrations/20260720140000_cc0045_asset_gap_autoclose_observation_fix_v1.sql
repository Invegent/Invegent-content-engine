-- Migration: cc0045_asset_gap_autoclose_observation_fix_v1
-- Lane: cc-0045 — auto-close AUDIT-OBSERVATION fix (PRODUCT_PROOF · T3). Depends on cc0044 (must be live).
-- Ships DARK (p_dry_run default true). Function body only; ZERO schema/table/index/grant change.
--
-- WHAT THIS FIXES (one line in the reconcile resolve path)
--   The cc-0044 reconcile writes the resolution breadcrumb as
--       INSERT m.asset_gap_observation (suggestion_id, source_post_id=latest_source_post_id, ...) ON CONFLICT DO NOTHING
--   but the DETECT side already wrote an observation on that EXACT (suggestion_id, latest_source_post_id) pair
--   (a detected suggestion always has one), so the arbiter index asset_gap_observation_suggestion_post_uidx
--   collides → DO NOTHING → the 'auto_resolved_gap_absent' breadcrumb is SUPPRESSED for EVERY real resolution.
--   (Missed by cc-0044's hermetic proof only because its seed had a NULL latest_source_post_id.)
--   FIX: write the resolution observation with source_post_id = NULL (NULLs are distinct in the btree → never
--   collides), carrying the resolving post in evidence_codes for provenance. NULL-post rows are already excluded
--   from the demand_count computation (WHERE source_post_id IS NOT NULL), so counts are unaffected.
--   The resolve itself (status='resolved' + resolved_client_asset_id + updated_at) was always correct — this
--   only restores the audit trail. EVERYTHING ELSE in run_asset_gap_analysis is byte-identical to cc-0044.
--
...
'cc-0043 dark asset-gap analyzer-write + cc-0044 reconcile close-pass + cc-0045 audit-observation fix: sweeps approved prepared drafts, calls analyze_asset_gap, fail-closed-validates, and persists ONLY detected=true demand into m.asset_gap_suggestion (+observation) via the live partial-unique signature; THEN reconciles status=open suggestions, flipping a row to resolved (attributing the now-satisfying c.client_brand_asset) when select_template=ok yields a selected asset for the demand slot. Governed terminal rule preserved (open-only scan; no silent reopen of resolved/dismissed). cc-0045: the resolution audit observation is written with source_post_id=NULL to avoid the detect-observation unique-index collision (resolving post carried in evidence_codes). Idempotent (resolved rows leave the scan set). pg_advisory_xact_lock per signature. p_dry_run default true = writes nothing. service_role only. Never approves/promotes/harvests/mutates drafts.';
...
begin
  ...
  or has_function_privilege('authenticated', 'public.run_asset_gap_analysis(int,int,boolean,text)', 'EXECUTE') then
    raise exception 'cc-0045 ACL assertion failed: anon/authenticated retain EXECUTE on run_asset_gap_analysis';
  end if;
  if not has_function_privilege('service_role', 'public.run_asset_gap_analysis(int,int,boolean,text)', 'EXECUTE') then
    raise exception 'cc-0045 ACL assertion failed: service_role missing EXECUTE on run_asset_gap_analysis';
  end if;
end
$assert$;
...
-- Restore the cc-0044 autoclose body verbatim by re-applying
--   supabase/migrations/20260720120000_cc0044_asset_gap_analyzer_autoclose_v1.sql
