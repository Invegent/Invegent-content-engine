...
--   _harness/cc_broll_consumption_sliceB_20260728/03_resolve_slot_assets_v1_4_broll_exclusive.sql (gitignored harness dir)
...
-- Slice B · Migration C (DDL) — resolve_slot_assets v1.4: EXCLUSIVE Background-by-element-type.
-- FIXES the v1.3 defect the proof caught: a video-typed Background field admitted image 'background'
-- assets ALONGSIDE broll, so the resolver could fill a VIDEO Background with a still image.
-- TWO changes vs v1.3:
--   (Anchor 2) client-asset Background predicate is now EXCLUSIVE by element type:
--       logo always · video Background → broll_background ONLY · image Background → 'background' ONLY.
--   (Shared-pool gate) a video Background never falls back to shared IMAGE static_backgrounds
--       (adds `AND NOT v_bg_is_video` to the fallback entry condition).
-- Everything else is byte-identical to v1.3. Downstream anchors 3a/3b/3c (broll→Background routing +
-- text-safety) are unchanged. Rollback of the whole lane = 01_ROLLBACK (verbatim v1.2 body).
