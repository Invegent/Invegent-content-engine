...
--
-- Two fix-ups vs original brief, applied in production via Supabase MCP and
-- mirrored here for repo/migration-history alignment:
--   FIX 1: c.client_content_scope column corrections (5 places)
--          - column is `vertical_id` (NOT `content_vertical_id`)
--          - table has NO `is_active` column; that filter removed entirely
--   FIX 2: m.post_draft.draft_body is NOT NULL with no default; skeleton uses
--          empty string placeholder. ai-worker fills the real body during
--          synthesis.
--
...
--        approval_status='draft', draft_body='' placeholder)
...
BEGIN
  ...
  LOOP
    ...
    IF v_decision IN ('filled','evergreen') THEN
      -- draft_body is NOT NULL on m.post_draft; use empty string as skeleton placeholder.
      -- ai-worker fills the real body during synthesis.
...
COMMENT ON FUNCTION m.fill_pending_slots(integer, boolean) IS
  'THE FILL FUNCTION (heart of v3). For each pending_fill slot whose window has opened: pool query → dedup → quality gates → optional pool-health relaxation → optional evergreen fallback → audit + skeleton draft (empty draft_body placeholder; ai-worker fills) + ai_job + slot transition (pending_fill → fill_in_progress per LD13). p_shadow controls whether produced drafts are shadow (default true). Returns jsonb {processed, shadow, results: [...], ran_at}. Stage 8.037 (fix-ups: c.client_content_scope columns + draft_body NOT NULL).';
