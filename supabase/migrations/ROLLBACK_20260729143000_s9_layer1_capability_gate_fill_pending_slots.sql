...
-- REPLACE takes a brief catalog lock but does not alter or abort a call already in
-- progress. An in-flight tick completes on the body it started with; the next tick
-- (at most ~10 minutes later) picks up this definition. There is no partial-execution
-- or mixed-body risk, and no need to pause the cron job. This reasoning applies
-- identically to the forward apply.
--
