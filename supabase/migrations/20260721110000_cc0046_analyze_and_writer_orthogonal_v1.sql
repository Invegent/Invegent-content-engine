...
-- ── cc-0046 Concern-1: resolve_slot_assets refactored to consume the shared authoritative
-- vertical-basis contract public.derive_template_vertical (Artifact 1). VALUE-IDENTICAL to the
-- live v1.2 body — only the inline template-tag->family-tag derivation is replaced by the shared
-- call, so resolver and probe cannot diverge. Proof: old<->new resolve body diff (additive/one-line).
-- Rollback: _harness/cc0046_ddl/rollback_functions.sql (byte-exact prior resolve v1.2 body).
...
BEGIN
  ...
  IF v_has_background
    ...
    IF 'vertical_shared' = ANY (v_permitted) THEN
      -- cc-0046 Concern-1: consume the ONE authoritative vertical-basis contract
      -- (public.derive_template_vertical) shared with probe_asset_inventory, instead of an inline
      -- duplicate derivation. VALUE-IDENTICAL (same template-tag -> family-tag fallback).
...
begin
  ...
  -- Concern-1: pass the EXACT candidate template whose resolution reached assets_fail_closed
  -- (v_cand_tid — the same template resolve_slot_assets was called with above). The probe derives
  -- the vertical via the SAME authoritative contract (derive_template_vertical) resolve consumes.
  v_probe := case when v_client_short and v_slot_kind in ('static_background','logo')
                  then public.probe_asset_inventory(p_client_slug, p_platform, v_cand_tid, v_slot_kind)
                  else null end;
