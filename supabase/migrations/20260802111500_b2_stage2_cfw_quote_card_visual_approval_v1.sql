-- =====================================================================
-- B2 Stage 2 — visual-approval promotion, Care for Welfare x generic_quote_card_1x1_v1
...
-- and stop at their individual apply gates."
--
...
-- combined packet, applies identically here): Care for Welfare's current
...
-- header did not name any execution channel at all).
--
-- (R) ROLLBACK: see the paired
--   ROLLBACK_20260802111500_b2_stage2_cfw_quote_card_visual_approval_v1.sql
-- =====================================================================

BEGIN;

CREATE TEMP TABLE b2_stage2_cfw_baseline ON COMMIT DROP AS
SELECT
...
BEGIN
  ...
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): expected exactly 1 row updated for assignment b2510001-...-000000000003 (care-for-welfare-pty-ltd x quote_card, status proposed), got %. Row may have already been promoted or moved to a different state — STOP, do not force.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
END $$;
...
BEGIN
  ...
  VALUES (
    ...
    now(),
    'PK visual verdict "three visual are good" 2026-08-02 (direct, this session) on B2 Stage-1 fresh preview render 605e602f-00ed-4fd0-aeb7-13e04dcf24ae (generic_quote_card_1x1_v1, Care for Welfare branding, template default placeholder copy — no real editorial content rendered). No live select_template winner change for this client (existing production_proven market_insight_card winner unaffected; this row becomes a new alternative). Rung 6 only; rungs 7-9 are separate future apply gates, NOT authorised here.'
  );
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): proof_event insert did not insert exactly 1 row, got %.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
END $$;
...
BEGIN
  ...
  WHERE id = 'b2510001-0000-4000-8000-000000000003'
    ...
    AND approved_by = 'PK';
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): post-update target-state assertion failed, got %.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
  ...
  IF v_rows <> 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): post-insert target-state assertion failed, got %.', v_rows
      USING ERRCODE = 'P0001';
  END IF;
END $$;
...
BEGIN
  SELECT baseline_visually_approved_count, baseline_proof_event_count
    INTO v_baseline_va, v_baseline_pe FROM b2_stage2_cfw_baseline;
  ...
  IF v_current_va <> v_baseline_va + 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): pool-neutrality assertion failed — visually_approved count changed by % (expected +1), baseline %, current %.', (v_current_va - v_baseline_va), v_baseline_va, v_current_va
      USING ERRCODE = 'P0001';
  END IF;
  IF v_current_pe <> v_baseline_pe + 1 THEN
    RAISE EXCEPTION 'B2 Stage-2 (CFW): pool-neutrality assertion failed — proof_event count changed by % (expected +1), baseline %, current %.', (v_current_pe - v_baseline_pe), v_baseline_pe, v_current_pe
      USING ERRCODE = 'P0001';
  END IF;
END $$;
