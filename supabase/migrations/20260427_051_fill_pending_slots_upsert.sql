...
--
-- Recovery cron resets slots to pending_fill but leaves orphan m.post_draft
-- rows. Bare INSERT then conflicts with LD18 unique index uq_post_draft_slot_id
-- (Stage 11.048). Fill cron has been failing every 10 min since 02:20 UTC.
--
...
--
-- Triggers verified safe under UPSERT-UPDATE:
--   - trg_handle_draft_rejection: AFTER UPDATE OF approval_status; only acts
--     on 'rejected'. We set 'draft'. No-op.
--   - trg_post_draft_updated_at: BEFORE UPDATE; sets updated_at = NOW().
--     We're already setting it; trigger override is fine.
--   - trg_release_queue_on_asset_ready: AFTER UPDATE OF image_status,
--     video_status; only fires when status LEAVES 'pending'. We set back
--     TO 'pending'. No-op.
--   - trg_prevent_published_draft_delete: BEFORE DELETE; we're not deleting.
--
-- ON CONFLICT spec MUST match the partial unique index predicate exactly:
--   ON CONFLICT (slot_id) WHERE (slot_id IS NOT NULL)
-- Postgres rejects ON CONFLICT specs whose predicate doesn't match.
--
-- Function body byte-identical to current definition (captured via
-- pg_get_functiondef at author time) EXCEPT for the post_draft INSERT block.
...
BEGIN
  ...
  LOOP
    ...
    IF v_decision IN ('filled','evergreen') THEN
      -- Stage 12.051 UPSERT: recovery cron resets slots to pending_fill but
      -- leaves orphan m.post_draft rows. Bare INSERT then conflicts with
      -- LD18 unique index uq_post_draft_slot_id. UPSERT path refreshes the
      -- existing draft row to skeleton state; audit trail preserved
      -- (post_draft_id stable, version++). draft_body NOT NULL → empty string.
      -- ON CONFLICT predicate matches the partial index WHERE clause exactly.
...
COMMENT ON FUNCTION m.fill_pending_slots(integer, boolean) IS
  'Fill pending_fill slots from the materialised pool. Stage 12.051: post_draft INSERT became UPSERT to handle recovery-loop case (orphan draft row left by m.recover_stuck_slots). ON CONFLICT (slot_id) WHERE (slot_id IS NOT NULL) matches the LD18 partial unique index predicate.';
