-- Migration: p1c_materialise_slots_honour_format_override
-- Purpose: Make m.materialise_slots honour c.client_publish_schedule.format_override.
--
-- Semantics:
--   * Token-only override: when a schedule row carries a non-NULL format_override,
--     it wins over BOTH the format-mix allocator (v_chosen) AND the legacy
--     preferred format (v_preferred_fmt) for that row's slots. It is applied as the
--     single-element format_preference on the inserted slot.
--   * Forward-only: the INSERT keeps ON CONFLICT DO NOTHING against
--     idx_slot_unique_active, so already-materialised active slots are NOT
--     retroactively re-stamped — a changed override only affects slots
--     materialised AFTER this migration (and after the override is set).
--   * No platform-support guard is added here. Validation of the override against
--     t."5.3_content_format".platform_support lives at the save RPC
--     (public.save_week_format_override), which is fail-closed at save time.
--
-- Additive edits to the byte-exact live base only:
--   1. top FOR loop SELECT: + cps.format_override
--   2. before INSERT INTO m.slot: apply v_rule.format_override when non-NULL
-- Everything else identical to the live base: SECURITY DEFINER
--   SET search_path TO 'public','pg_temp', BOTH EXTRACT(dow ...) joins
--   (Sunday=0 Postgres dow convention — NOT isodow), ON CONFLICT DO NOTHING.
--
-- Base pre-image md5 (pg_get_functiondef, live 2026-07-27):
--   m.materialise_slots(integer) = 48e2db58c8696f091e60051321a1fcb8
--
-- Rollback: docs/briefs/artifacts/p1c-rollback.sql
-- ============================================================================
...
BEGIN
  FOR v_rule IN
    SELECT cps.schedule_id, cps.client_id, cps.platform, cps.day_of_week, cps.publish_time, cps.format_override
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_rule.format_override IS NOT NULL THEN
        v_format_pref := ARRAY[v_rule.format_override];
      END IF;
