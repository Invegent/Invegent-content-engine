-- =====================================================================================
-- ICE — B-roll Rotation Governance v1 — ROLLBACK
-- Pairs with: broll-rotation-governance-v1-migration.sql
-- WRITTEN AND DRY-RUN VALIDATED BEFORE APPLY (T3 requirement).
--
-- IDENTITY CHECK (apply/rollback pairing): the apply packet creates exactly four things
--   1. c.geo_class                      -> dropped here
--   2. c.client_format_copy_geography   -> dropped here
--   3. public.geo_relation(text,text)   -> dropped here
...
-- state needs restoring. The live template winner, the parity overlay, the video-worker
-- and every voice/music/logo binding are untouched in both directions.
-- =====================================================================================

BEGIN;

-- -------------------------------------------------------------------------------------
-- STEP 1 (FIRST): restore resolve_slot_assets to the VERBATIM v1.4 body.
-- Captured live from pg_get_functiondef before the apply. CREATE OR REPLACE preserves
-- the ACL (postgres=X/postgres | service_role=X/postgres) in this direction too.
-- -------------------------------------------------------------------------------------
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE cba.client_id = v_client_id
      -- ANCHOR 2 (v1.4): Background is EXCLUSIVE by element type. Logo always; a VIDEO Background
      -- takes ONLY broll_background; an IMAGE Background takes ONLY image 'background'. This prevents a
      -- video Background field from ever resolving a still image (the v1.3 defect the proof caught).
  ...
  LOOP
    -- ANCHOR 3a: route broll_background to the Background slot.
    ...
    ELSE
      ...
      END IF;
      -- ANCHOR 3b: text-safety gate also applies to broll_background.
    ...
    -- ANCHOR 3c: route broll_background into the Background candidate arrays.
  ...
  IF v_has_background
     AND NOT v_bg_is_video          -- v1.4: a VIDEO Background never falls back to shared IMAGE backgrounds
...
-- -------------------------------------------------------------------------------------
-- STEP 2 (ONLY AFTER STEP 1): drop the helper and the two governance tables.
-- Safe now — no installed function references them.
-- -------------------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.geo_relation(text, text);
DROP TABLE IF EXISTS c.client_format_copy_geography;
DROP TABLE IF EXISTS c.geo_class;

-- -------------------------------------------------------------------------------------
-- STEP 3: post-rollback assertion — fail closed if the resolver did not actually revert.
-- v1.5 is identified by its 'broll_selection' return key; v1.4 has no such key.
-- -------------------------------------------------------------------------------------
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'resolve_slot_assets'
      AND pg_get_functiondef(p.oid) LIKE '%broll_selection%'
  ) THEN
    RAISE EXCEPTION 'ROLLBACK ASSERTION FAILED: resolve_slot_assets still contains v1.5 markers';
  END IF;
