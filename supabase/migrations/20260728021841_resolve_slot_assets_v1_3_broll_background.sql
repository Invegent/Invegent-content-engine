...
--   _harness/cc_broll_consumption_sliceB_20260728/01_resolve_slot_assets_v1_3_broll_background.sql (gitignored harness dir)
...
-- Slice B · Migration A (DDL) — resolve_slot_assets v1.3: scoped broll_background support.
-- SCOPED, LEAK-FREE: broll_background is admitted ONLY when the template's Background field is
-- element_type='video' (v_bg_is_video). No existing registered background field is video-typed,
-- so this returns broll clips ONLY for the new video template — zero leak into image backgrounds.
-- Four anchors vs v1.2: DECLARE v_bg_is_video; needs-detection SELECT; client-asset WHERE;
-- and the three downstream asset_usage='background' sites (slot map / text-safety gate / bg routing).
-- Shared-pool fallback loop (asset_kind='static_background') is BYTE-IDENTICAL to v1.2 — no broll there.
-- Rollback: re-apply 01_ROLLBACK_resolve_slot_assets_v1_2.sql (verbatim v1.2 body).
-- Lineage: supabase/migrations/20260720150000_resolve_slot_assets_v1_2_shared_pool_fallback.sql
...
DECLARE
  ...
  v_bg_is_video    boolean := false;   -- ANCHOR 0 (v1.3): video-typed Background field present?
...
BEGIN
  ...
  -- ANCHOR 1 (v1.3): also capture whether the Background field is video-typed.
  SELECT
  ...
  FOR r IN
    ...
    WHERE cba.client_id = v_client_id
      -- ANCHOR 2 (v1.3): admit broll_background ONLY when the template Background field is video-typed.
  ...
  LOOP
    -- ANCHOR 3a (v1.3): route broll_background to the Background slot.
    ...
    ELSE
      ...
      END IF;
      -- ANCHOR 3b (v1.3): text-safety gate also applies to broll_background.
    ...
    -- ANCHOR 3c (v1.3): route broll_background into the Background candidate arrays.
