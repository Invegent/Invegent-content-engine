...
-- Total UPDATEs: 83 (69 in c, 14 in f)
-- Apply via: chat → Supabase MCP apply_migration (per D170). CC does NOT call apply_migration.
-- Idempotent: each UPDATE matches by (table_id, column_name) so re-running is safe.
...
UPDATE k.column_registry SET column_purpose = $cp$Whether stakeholder consent must be captured/verified before this synthetic avatar is used in published video. False permits use without an explicit consent artefact (e.g. internal-only stakeholder).$cp$, updated_at = NOW()
...
UPDATE k.column_registry SET column_purpose = $cp$Lifecycle of the avatar generation job at HeyGen. 'empty' is the only observed value to date — additional states such as 'pending', 'generating', 'ready', 'failed' are likely in design; full set should be confirmed against HeyGen worker code if precision matters.$cp$, updated_at = NOW()
WHERE table_id = (SELECT table_id FROM k.table_registry WHERE schema_name = 'c' AND table_name = 'brand_avatar') AND column_name = 'avatar_gen_status';
...
UPDATE k.column_registry SET column_purpose = $cp$Lifecycle status of the production avatar+voice profile for this client. No populated rows yet — values not observed; column reserved for future video pipeline activation. Likely lifecycle: 'pending' → 'consent_signed' → 'active'.$cp$, updated_at = NOW()
WHERE table_id = (SELECT table_id FROM k.table_registry WHERE schema_name = 'c' AND table_name = 'client_avatar_profile') AND column_name = 'avatar_status';

UPDATE k.column_registry SET column_purpose = $cp$Whether the client has consented to a custom voice clone (ElevenLabs Voice Cloning) for video synthesis. False = use a stock ElevenLabs voice instead.$cp$, updated_at = NOW()
...
UPDATE k.column_registry SET column_purpose = $cp$Asset classification. Per table_purpose the canonical set is 'logo', 'banner', 'avatar' — image-worker reads asset_type to pick the right asset for a given Creatomate render slot. No populated rows yet.$cp$, updated_at = NOW()
WHERE table_id = (SELECT table_id FROM k.table_registry WHERE schema_name = 'c' AND table_name = 'client_brand_asset') AND column_name = 'asset_type';
...
UPDATE k.column_registry SET column_purpose = $cp$Whether the platform supports markdown formatting in posts (LinkedIn yes; Facebook/Instagram no). Drives whether ai-worker emits markdown or plain text in the draft.$cp$, updated_at = NOW()
...
UPDATE k.column_registry SET column_purpose = $cp$Whether the slot-driven R6 publishing path applies to this client x platform. When true, the slot-materialiser and slot-promoter chain handles publishing; when false, the legacy publishing path runs instead.$cp$, updated_at = NOW()
...
UPDATE k.column_registry SET column_purpose = $cp$Whether this support item is featured for the client. Featured items receive higher weighting in bundler signal scoring (the operator has flagged them as a content-priority service).$cp$, updated_at = NOW()
...
-- ── c.external_reviewer ──────────────────────────────────────────────────
UPDATE k.column_registry SET column_purpose = $cp$Whether this external reviewer identity is currently active. The D156 reviewer layer is paused per D162; rows preserved for audit, no new reviews routed.$cp$, updated_at = NOW()
...
UPDATE k.column_registry SET column_purpose = $cp$Whether this reviewer routing rule is currently active. D156 reviewer layer paused per D162; rules retained for future re-activation.$cp$, updated_at = NOW()
...
-- ── f.raw_metric_point ───────────────────────────────────────────────────
UPDATE k.column_registry SET column_purpose = $cp$Type of the entity the metric describes (e.g. 'page', 'post', 'account', 'video'). No populated rows yet — values not observed; column reserved for future analytics ingestion. Combined with entity_key to identify the metric subject.$cp$, updated_at = NOW()
WHERE table_id = (SELECT table_id FROM k.table_registry WHERE schema_name = 'f' AND table_name = 'raw_metric_point') AND column_name = 'entity_type';
